...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2025-09-10'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0112_Vol02_change_006'"/>
    <xsl:param name="editingDesc" as="xs:string" select="'Tagging foreign (la).'"/>
...
<!--
Foreign (la) tagging in large spanish texts

IMPORTANT:

Add latin words to the file latinWords.xml, which do not appear in Spanish e.g.: etiam, hoc, Deus.

Be careful with words like "sed" which are used in both languages.

After processing check:
1 Are there mixed passages (spanish and Latin) in t:i //foreign[@resp and matches(.,' y | en | para ')]]
2 Check there is no embeded foreig. //foreign//foreign--> 
    ...
    <!--List of latin words, prepositions etc. with boundaries: white spaces or \n.-->
    <xsl:variable name="latinWords" select="doc('latinWords.xml')//word"/>
    <xsl:variable name="personsSorted" as="xs:string*">
        <xsl:for-each select="$latinWords">
            <xsl:sort order="descending" select="string-length(.)"/>
            <xsl:value-of select="."/>
        </xsl:for-each>
    </xsl:variable>

<!-- every entry separated by pipe | to make them look like regular expressions -->

    <xsl:variable name="regex1" select="string-join($latinWords, '|')"/>
    
    <xsl:template match="t:i[not(ancestor::tei:foreign[@xml:lang eq 'la']) and matches(.,$regex1)]">
