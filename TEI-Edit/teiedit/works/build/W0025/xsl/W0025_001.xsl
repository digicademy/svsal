...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2026-03-18'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0025_change_002'"/>
    <xsl:param name="editingDesc" as="xs:string" select="'Tagged @n, @type, @xml:id(s) in div(s), list(s), note/ref and @target in TOC and summaries'"/>
...
<!--Added @n, @type in div1-->
    <xsl:template match="tei:body//tei:div1[@type eq 'question']">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="n" select="count(preceding::tei:div1[@type eq 'question'][ancestor::tei:body = current()/ancestor::tei:body])+1"/>
    ...
    <xsl:template match="tei:body//tei:div3[not(@type)]">
        <xsl:copy>
            ...
            <xsl:attribute name="type">
                <xsl:choose>
                    <xsl:when test="child::tei:p[1][@rend = 'h3'][matches(.,'SVMMA')]">
                    <xsl:value-of select="'section'"/>
                    </xsl:when>
                    <xsl:when test="child::tei:p[1][@rend = 'h3'][matches(.,'COMMENTARIVM')]">
                    <xsl:value-of select="'commentary'"/>
                    </xsl:when>
                    <xsl:when test="child::tei:p[1][not(@rend)]">
                    <xsl:value-of select="'section'"/>
                    </xsl:when>
                    <xsl:otherwise><xsl:message terminate="no">Warning: div3 with different @type.</xsl:message></xsl:otherwise><!-- //tei:body//tei:div3[not(@type)] | //tei:body//tei:div3[@type eq ''] -->
                </xsl:choose>
            </xsl:attribute>
            <xsl:apply-templates/>
        </xsl:copy>
...
<!--TOC Added @target in ref-->   
    ...
    <xsl:template match="tei:div1[@type='contents'][1]//tei:ref[starts-with(., 'Q')]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="q-id"  select="count(preceding::tei:ref[starts-with(.,'Q')])+1"/>
            <xsl:attribute name="target" select="concat('#q',$q-id)"/>  
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
...
<!-- ref from content to div4 "Doubt-level" -->
    <xsl:template match="//tei:list[@xml:id='disputatio']//tei:ref[starts-with(., 'Du') or starts-with(., 'I') or starts-with(., 'V') or starts-with(., 'X')]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="dub-id"  select="count(preceding::tei:ref[ancestor::tei:list[@xml:id='disputatio']][starts-with(.,'Du') or starts-with(., 'I') or starts-with(., 'V') or starts-with(., 'X')])+1"/>
