...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="tei:ref">
        <xsl:copy>
            <xsl:copy-of select="@resp|@cert|@change|@n|@type|@xml:lang"/>
            <xsl:if test="@target">
    ...
    <xsl:template match="tei:cb">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:copy>
                    <xsl:if test="@n">
                        <!--<xsl:message select="'WARN: Found cb/@n: refers to actual column number?'"/> this work has actual cb/@n-->
                    </xsl:if>
    ...
    <xsl:template match="text()" priority="2">
        ...
        <xsl:choose>
            ...
            <xsl:when test="some $char in $interestingChars satisfies contains(., $char)">
                <!--<xsl:message terminate="no" select="concat('Info: encountered special character in text node: ', .)"/> In this work there are asterisks-->
                <xsl:value-of select="."/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="."/>
