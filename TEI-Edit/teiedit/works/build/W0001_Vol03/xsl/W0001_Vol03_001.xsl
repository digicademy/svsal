...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2019-06-17'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0001_Vol03_change_xxx'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Contents and Index in nested lists'"/>
    ...
    <xsl:template match="tei:body//tei:list//tei:item[not(descendant::tei:list)]">
        <xsl:copy>
            <xsl:choose>
    ...
    <xsl:template match="tei:list//tei:item//tei:p[1][not(ancestor::tei:argument)]">
            <xsl:element name="head">
                <xsl:apply-templates/>
            </xsl:element>
    </xsl:template>
    <xsl:template match="tei:list//tei:item//tei:p[preceding-sibling::tei:p]">
