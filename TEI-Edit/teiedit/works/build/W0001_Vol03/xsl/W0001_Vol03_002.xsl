...
<!--Added missing pagination (pb/@n).   -->
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2019-06-21'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0001_Vol03_change_xxx'"/>
    ...
    <xsl:template match="tei:body/tei:div1[1]//tei:pb[not(@sameAs)]">
    ...
    <xsl:template match="tei:body/tei:div1[2]//tei:pb[not(@sameAs)]">
        <xsl:variable name="n" as="xs:integer" select="count(./preceding::tei:pb[not(@sameAs)][ancestor::tei:body])+1"/>
        <xsl:copy>
            <xsl:copy-of select="@* except @n"/>
            <xsl:message>Adding @n to pb</xsl:message>
    ...
    <xsl:template match="tei:body/tei:div1[3]//tei:pb[not(@sameAs)]">
        <xsl:variable name="n" as="xs:integer" select="count(./preceding::tei:pb[not(@sameAs)][ancestor::tei:body])+1"/>
        <xsl:copy>
            <xsl:copy-of select="@* except @n"/>
            <xsl:message>Adding @n to pb</xsl:message>
