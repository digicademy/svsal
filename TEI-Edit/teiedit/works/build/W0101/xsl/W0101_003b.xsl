...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="tei:ref">
        <xsl:copy>
            ...
            <xsl:if test="following-sibling::*[1]/self::tei:note[@type='margin' and @rend = ('asterisk', 'dagger')]">
                <!--<xsl:variable name="xmlId" as="xs:string" select="generate-id(.)"/>-->
                <xsl:variable name="xmlId" as="xs:string" select="following-sibling::*[1]/self::tei:note/@xml:id[1]"/>
    ...
    <xsl:template match="tei:note[@type='margin' and @rend = ('asterisk', 'dagger')]">
        ...
        <xsl:copy>
            ...
            <xsl:copy-of select="@resp|@cert|@change|@xml:lang|@xml:id"/>
            <!-- if there are no p sub elements, wrap the whole note content in a single p -->
            <xsl:choose>
                <xsl:when test="not(descendant::tei:p)">
                    <xsl:element name="p">
