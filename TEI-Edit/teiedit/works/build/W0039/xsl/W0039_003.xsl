...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2020-04-28'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0039_change_017'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Tagged @type(s) to div(s) and @target to ref.'"/>
...
<!-- Added @target to ref(s).  -->
    <xsl:template match="tei:ref[ancestor::tei:div1[@type eq 'contents']]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="target" select="count(preceding::tei:ref)+1"/>
...
<!-- Added @xml:id and @type to div(s).  -->
    <xsl:template match="tei:div1[@type eq 'dict']//tei:div2[not(@type)]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
    ...
    <xsl:template match="tei:div3[not(@type) and ancestor::tei:div1[@type eq 'dict']]">
         <xsl:copy>
             <xsl:copy-of select="@*"/>
             <xsl:attribute name="type" select="'entry'"/>
             <xsl:attribute name="xml:id" select="concat('entry',count(preceding::tei:div3[ancestor::tei:div1[@type eq 'dict']])+1)"/>
             <xsl:apply-templates/>
         </xsl:copy>
     </xsl:template>
    <xsl:template match="tei:div1[@type eq 'dict']//tei:div4[not(@type)]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:attribute name="type" select="'entry'"/>
            <xsl:apply-templates/>
    ...
    <xsl:template match="tei:div1[@type eq 'dict']//tei:div5[not(@type)]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
