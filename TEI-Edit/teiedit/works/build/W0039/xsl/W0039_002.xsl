...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2020-01-16'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0039_change_016'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Correct hi @rend.'"/>
...
<!-- Replaced hi @rend eq 'right' by seg  @rend eq 'gap' -->
    <xsl:template match="tei:div1[@type eq 'contents']//tei:hi[@rend eq 'right']">
    ...
    <xsl:template match="tei:hi[@rend eq 'tao:W+1']|tei:hi[@rend eq 'tao:A']">
        <xsl:apply-templates/>
    </xsl:template>
    <xsl:template match="tei:hi[@rend eq 'tao:huifu:']">
