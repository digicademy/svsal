...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2020-01-14'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0039_change_015'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Added missing pagination @n in pb.'"/>
    ...
    <!-- Added @n to front recto, verso pages. -->
    <xsl:template match="tei:front//tei:pb[not(@sameAs)]" mode="round1">
         <xsl:copy>
             <xsl:copy-of select="@*"/>
             <xsl:choose>
                 <xsl:when test="@n eq ''">
                     <xsl:variable name="versoN" as="xs:string">
                         <xsl:value-of select="substring-before(preceding::tei:pb[1][not(@sameAs)]/@n,'r')"/>
                     </xsl:variable>
                     <xsl:attribute name="n" select="concat($versoN, 'v')"/>
                 </xsl:when>
                 <xsl:otherwise/>
             </xsl:choose>
    ...
    <!-- Added @n to recto pages. -->

    <xsl:template match="tei:body//tei:pb[not(@sameAs)]" mode="round1">
         <xsl:copy>
             <xsl:copy-of select="@*"/>
             <xsl:choose>
                 ...
                 <xsl:when test="@n eq ''">
                     <xsl:variable name="versoN" as="xs:string">
                         <xsl:value-of select="following::tei:pb[1][not(@sameAs)]/@n"/>
                     </xsl:variable>
                     <xsl:attribute name="n" select="concat($versoN, 'r')"/>
                 </xsl:when>
                 <xsl:otherwise/>
             </xsl:choose>
             <xsl:apply-templates mode="round1"/>
    ...
    <!-- ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ -->
    <!-- correct @n recto / verso pages written by tustep -->
    ...
    <xsl:template match="tei:body//tei:pb[not(@sameAs)]" mode="round2">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:choose>
                <xsl:when test="@n[matches(.,'\sv.r$')]">  
                    <xsl:variable name="NewRecto" as="xs:string">
                        <xsl:value-of select="substring-before(@n,'v.r')"/>
                    </xsl:variable>
                    <xsl:attribute name="n" select="concat(translate($NewRecto,' ',''),'r')"/>
                </xsl:when>
                <xsl:when test="@n[matches(.,'\sv.$')]">  
                    <xsl:variable name="NewVerso" as="xs:string">
                        <xsl:value-of select="substring-before(@n,' v.')"/>
                    </xsl:variable>
                    <xsl:attribute name="n" select="concat('[',translate($NewVerso,' ',''),']','v')"/>
                </xsl:when>
                <xsl:otherwise/>
            </xsl:choose>
