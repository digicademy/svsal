...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2019-03-11'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0007_change_028'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Extended tei:ref in table of contetns to full tei:item, and added list/@type within index.'"/>
    ...
    <xsl:template match="tei:div[@type eq 'contents']//tei:item[count(.//tei:ref) eq 1 and count(.//tei:pb) eq 0]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
    ...
    <xsl:template match="tei:div[@type eq 'contents']//tei:item[count(.//tei:ref) eq 1 and count(.//tei:pb) eq 0]//tei:ref">
        <xsl:apply-templates/>
    </xsl:template>
    
    <xsl:template match="tei:div[@type eq 'index']//tei:list[not(@type)]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
