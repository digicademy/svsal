...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2019-03-08'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0007_change_026'"/>
        ...
        <xsl:copy>
            ...
            <xsl:choose>
                <xsl:when test="@sameAs">
                    ...
                    <xsl:if test="not($targetPb/@facs)">
                        <xsl:message terminate="yes" select="concat('Error: pb targeted through @sameAs lacks @facs: ', $targetPb/@xml:id)"/>
                    </xsl:if>
    ...
    <!-- byline and argument receive no @xml:id for now -->
