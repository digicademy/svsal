...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0007_change_0014'"/>
    <xsl:param name="specialCharFile" as="xs:string" select="'../../../resources/chars/Sonderzeichen_2018-07-06.xml'"/>
                ...
                <xsl:analyze-string select="." regex="{$specialCharsRegex}"> 
                    ...
                    <xsl:non-matching-substring>
                        ...
                        <xsl:if test="matches(., '[&#x0100;-&#x10ffff;]')">
                            <xsl:variable name="lineN" select="count($thisNode/preceding::tei:lb intersect $thisNode/preceding::tei:pb[1]/following::tei:lb)"/>
                            <xsl:message terminate="yes" select="concat('Error: found undeclared special character in text node: ', ., '; on page ', 
                                                                         (if ($thisNode/preceding::tei:pb[1]/@facs) then $thisNode/preceding::tei:pb[1]/@facs else $thisNode/preceding::tei:pb[1]/@sameAs), 
                                                                         ', line ', string($lineN))"/>
                        </xsl:if>
                        <xsl:value-of select="."/>
                    </xsl:non-matching-substring>
