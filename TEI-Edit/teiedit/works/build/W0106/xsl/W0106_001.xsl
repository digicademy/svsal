...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2019-11-13'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0106_change_007'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Added missing pagination and renamed facsimiles.'"/>
    ...
    <xsl:template match="tei:pb[not(@sameAs)]">
        <xsl:copy>
            <xsl:copy-of select="@n[ancestor::tei:front]|@xml:id|@type"/>
            <xsl:choose>
                <!-- @n without any value are verso pages -->
                <xsl:when test="@n eq ''">
                    <xsl:variable name="rectoN" as="xs:string" select="preceding::tei:pb[not(@sameAs)][1]/@n"/>   
                    <xsl:attribute name="n">
                        <xsl:choose>
                            <xsl:when test="$rectoN[matches(.,'^\d')]">
                                <xsl:value-of select="concat('[', $rectoN, ']v')"/>
                            </xsl:when>
                            <xsl:when test="$rectoN[matches(.,'^\[')]">
                                <xsl:value-of select="concat($rectoN,'v')"/>
                            </xsl:when>
                            <xsl:otherwise/>
                        </xsl:choose>
            ...
            <xsl:attribute name="facs">
                <xsl:variable name="Facs">
                    <xsl:number value="count(preceding::tei:pb[not(@sameAs)]) + 5" format="0000"/></xsl:variable>
                <xsl:value-of select="concat('W0106-',$Facs)"/>
            </xsl:attribute>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
