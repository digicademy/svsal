...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2019-11-27'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0106_change_014'"/>
    <xsl:param name="editingDesc" as="xs:string" select="'Annotated hyphenated breaks interrupted by hi and label.'"/>
    ...
    <xsl:function name="local:hyphens" as="xs:boolean">
        ...
        <xsl:value-of select="not($lb/ancestor::tei:label[@type eq 'margin']) and $lb/preceding-sibling::*[1]/self::tei:label[@type eq 'margin'] and $lb/preceding-sibling::*[2]/self::tei:hi[matches((.//text())[last()],'[-=]$')]"/>
    </xsl:function>
    
    <xsl:template match="text()[matches(.,'[-=]\s*$') and ./parent::tei:hi/following-sibling::*[1]/self::tei:label[@type eq 'margin'] and ./parent::tei:hi/following-sibling::*[2]/self::tei:lb[local:hyphens(.)]]" mode="round1">
    ...
    <xsl:template match="tei:pb[not(@break)]" mode="round2">
        ...
        <xsl:choose>
            ...
            <xsl:when test="./following-sibling::*[1]/self::tei:cb and ./following-sibling::*[2]/self::tei:lb[@break eq 'no']">
                <xsl:attribute name="break" select="'no'"/>
                <xsl:attribute name="rendition" select="'#hyphen'"/>
            </xsl:when>
        </xsl:choose>
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="geloscht" select="count(//text()[matches(.,'[=-]\s*$') and ./parent::tei:hi/following-sibling::*[1]/self::tei:label[@type eq 'margin'] and ./parent::tei:hi/following-sibling::*[2]/self::tei:lb[local:hyphens(.)]])"/>
