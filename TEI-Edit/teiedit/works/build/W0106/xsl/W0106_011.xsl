...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2019-11-28'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0106_change_017'"/>
    ...
    <xsl:key name="label" match="tei:label[@place='margin']" use="@xml:id"/>
    ...
    <xsl:key name="mlabel" match="tei:label[@place='margin']" use="@xml:id"/>
    ...
    <xsl:template match="tei:text//tei:label[@place eq 'margin']">
        <xsl:copy>
            <xsl:copy-of select="@* except @xml:id"/>
            <xsl:attribute name="xml:id">
                <xsl:value-of select="sal:mkAnchorElemId(., 'lm')"/>
            </xsl:attribute>
        <xsl:if test="@sameAs">
                <xsl:variable name="targetLabel" select="key('label', substring-after(@sameAs, '#'))"/>
                <xsl:choose>
                    <xsl:when test="$targetLabel">
                        <xsl:attribute name="sameAs" select="concat('#', sal:mkAnchorElemId($targetLabel, 'lm'))"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:message terminate="yes" select="concat('Error: could not find a valid target for label/@sameAs:', @sameAs)"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:if>
            <xsl:apply-templates/>
    ...
    <xsl:template match="tei:text//tei:ref[@target and not((substring(@target, 15, 4) eq '_mn_') or (substring(@target, 15, 4) eq '-nm-'))]">
        <xsl:copy>
            ...
            <xsl:variable name="label" select="key('label', substring-after(@target, '#'))"/>
            ...
            <xsl:choose>
                <xsl:when test="$milestone or $label or $p or $div or $list or $item or $mnote or $pb ">
                    <xsl:choose>
                        <xsl:when test="$milestone">
                            <xsl:attribute name="target" select="concat('#', sal:mkAnchorElemId($milestone, 'mi'))"/>
                        </xsl:when>
                        <xsl:when test="$label">
                            <xsl:attribute name="target" select="concat('#', sal:mkAnchorElemId($label, 'lm'))"/>
                        </xsl:when>
                        <xsl:when test="$p">
    ...
    <xsl:template match="tei:text//tei:ref[(substring(@target, 15, 4) eq '_mn_') or (substring(@target, 15, 4) eq '-nm-') or (substring(@target, 15, 4) eq '-lm-')]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
