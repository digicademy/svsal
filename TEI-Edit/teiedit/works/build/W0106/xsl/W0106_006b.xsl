...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="tei:item[not(@type eq 'inline')]">
        <xsl:copy>
    ...
    <xsl:template match="tei:label[@type eq 'margin']">
        <xsl:copy>
            <xsl:copy-of select="@* except @type"/>
                ...
                <xsl:when test="@facs">
                    <!--<xsl:variable name="pageURN">
                        <xsl:choose>
                            ...
                            <xsl:when test="matches(./@facs, '^W[0-9]{4}-[0-9]{4}$')">
                                <xsl:value-of select="."/>
                            </xsl:when>
                            <xsl:otherwise>
                    ...
                    <xsl:attribute name="facs">
                        <xsl:value-of select="concat('facs:', @facs)"/>
                    </xsl:attribute>
                </xsl:when>
    ...
    <xsl:template match="*[not(self::tei:hi or self::tei:p or self::tei:seg)][@rend or @rendition]">
