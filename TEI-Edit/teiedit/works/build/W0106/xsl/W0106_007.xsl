...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0106_change_013'"/>
    ...
    <xsl:function name="local:isHyphenation" as="xs:boolean">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$elem/preceding-sibling::node()[not(self::text() and normalize-space() eq '')][1][self::tei:note or self::tei:label[@type eq 'marginalia' or 'margin']]">
    ...
    <xsl:function name="local:removeHyphen">
        <xsl:param name="textNode" as="text()"/>
            <xsl:choose>
                ...
                <!-- TODO: include label[@type eq 'margin'] in here -->
                <xsl:when test="matches($textNode, '[-=]\n?$') 
                                and $textNode/following-sibling::node()[1][self::tei:note or self::tei:label[@type eq 'marginalia' or 'margin']]
