...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2021-05-31'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0004_change_037'"/>
    <xsl:param name="editingDesc" as="xs:string" select="'Generated @xml:id after post corrections.'"/>
    ...
    <xsl:key name="pb" match="tei:pb" use="@sameAs"/>
    ...
    <xsl:template match="tei:text//tei:p">
        <xsl:copy>
            <xsl:copy-of select="@* except  @part|@xml:id"/> 
        ...
        <xsl:copy>
            ...
            <xsl:choose>
                <xsl:when test="@sameAs">
                    ...
                    <xsl:attribute name="sameAs" select="concat('#', $targetPb)"/>
                    <!-\-<xsl:attribute name="xml:id" select="sal:mkPbId(current(), $targetPb/@facs)"/>-\->
                    <xsl:attribute name="xml:id" select="concat(current(), $targetPb)"/>
                </xsl:when>
                <xsl:otherwise>
