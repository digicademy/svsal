...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2021-05-06'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0018_change_005'"></xsl:param>
...
<!-- 
        ...
        * Every template has a case and a mode:
        ...
        Taken from W0005_Vol01_005.xsl only for W0018:
        
        26) "q̃", que - algun mode="only-que"
        29) only for W0018 "đ| (char0111|charf159)- de" mode="only-de"
    ...
    | How to add new cases:  |
     ...
     1) Test the new pattern in spanisch texts in version 001 (W0005_Vol01, W0017, W0061, W0083 etc.)
        Words found should neather yield exceptions, ambiguities nor show conflicts with other cases in this program.  
     ...
     5) Write a template that matches only text in spanisch, which is not tagged as expansion yet and add the new mode:
          ...
          Regex-groups must be place in () and distributed in the new elements. See the templates below.
...
<!--7) ẽ + "cia" e.g. "differẽcia"-->
    ...
    <xsl:template match="text()[not(ancestor::tei:abbr or ancestor::*[@xml:lang = ('la','grc','gr','he','fr','pt','it')])]" mode="encia">
        <xsl:analyze-string select="." regex="{'(\s)([aA-zZſçñ]+)(ẽ|ẽ|ē)(cia)([ \.,;\(\)])'}"><!--(\w+[^ãẽõũ])-->
            <xsl:matching-substring>
                <xsl:value-of select="regex-group(1)"/>
                <xsl:element name="abbr">
                    <xsl:attribute name="rend" select="'choice'"/>
...
<!--26) only for W0018 "q̃", que - algun mode="only-que"-->
    
    <xsl:variable name="only-que">
        <xsl:apply-templates select="$prow" mode="only-que"/>
    </xsl:variable>
...
<!--29) only for W0018 "đ| (char0111|charf159)- de" mode="only-de"-->
    ...
    <xsl:variable name="only-de">
        <xsl:apply-templates select="$only-que" mode="only-de"/>
    </xsl:variable>
