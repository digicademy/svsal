...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0096_Vol02_change_0011'"/>
    <xsl:param name="specialCharFile" as="xs:string" select="'../../../resources/chars/specialchars_2019-10-25.xml'"/>
            ...
            <xsl:when test="(normalize-space(.) != '') and not(ancestor::tei:g or ancestor::tei:foreign[@xml:lang=('gr', 'gre', 'grc')])">
                <xsl:analyze-string select="." regex="{$specialCharsRegex}"> 
                    ...
                    <xsl:non-matching-substring>
                        <!-- check if there are any further special chars not (yet) declared in the charDecl, $this/preceding::tei:pb[1]/@facs, $this/preceding::tei:lb[1]/@xml:id -->
                        <xsl:if test="matches(., '[&#x0100;-&#x10ffff;]')">
                            <xsl:message terminate="yes" select="concat('Error: found undeclared special character in text node: ', $this/preceding::tei:pb[1]/@facs)"/>
                        </xsl:if>
                        <xsl:value-of select="."/>
                    </xsl:non-matching-substring>
                </xsl:analyze-string>
