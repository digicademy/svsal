...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="tei:pb">
        <xsl:copy>
            <xsl:if test="(not(@n) or normalize-space(@n) eq '' or not(matches(@n, '\[?[\dxivjl]{1,7}\]?[rv]?'))) and not(@sameAs)">
    ...
    <xsl:template match="t:colShift[xs:integer(@t:cols) gt 1]">
        <xsl:element name="cb">
            ...
            <xsl:if test="following-sibling::*[1]/self::tei:cb[@sameAs]">
                    <!--<xsl:attribute name="sameAs" select="following-sibling::*[1]/self::tei:cb/@sameAs"/>-->
                
                <xsl:variable name="sameAsCb" select="key('anyNode', substring-after(./following-sibling::*/self::tei:cb/@sameAs, '#'))"/>
                    <xsl:choose>
                        <xsl:when test="$sameAsCb">
                            <xsl:attribute name="sameAs" select="concat('#', generate-id($sameAsCb))"/>
                        </xsl:when>
                        <xsl:otherwise>
    ...
    <xsl:template match="tei:cb[not(preceding-sibling::*[1]/self::t:colShift[xs:integer(@t:cols) gt 1])]">
        
        <xsl:copy>
            <xsl:copy-of select="@n|@resp|@cert|@change|@sameAs"/>
            <!--<xsl:attribute name="n" select="count(preceding::tei:pb[1]/following::tei:cb intersect preceding::tei:cb) + 1"/>-->
            <xsl:attribute name="xml:id" select="generate-id(.)"/>
            <xsl:if test="@sameAs">
                <xsl:variable name="sameAsCb" select="key('anyNode', substring-after(@sameAs, '#'))"/>
                <xsl:choose>
                    <xsl:when test="$sameAsCb">
                        <xsl:attribute name="sameAs" select="concat('#', generate-id($sameAsCb))"/>
                    </xsl:when>
    ...
    <xsl:template match="tei:hi">
        <xsl:choose>
            ...
            <xsl:when test="@rend = ('init', 'right', 'center', 'sp','recte')">
                <xsl:choose>
