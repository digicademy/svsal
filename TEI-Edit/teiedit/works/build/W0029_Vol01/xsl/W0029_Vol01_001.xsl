...
<xsl:param name="editors" as="xs:string" select="'#DG #PS #auto'"/>
    <xsl:param name="editingDate" as="xs:string" select="'2024-07-25'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0029_Vol01_change_002'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Adding @xml:id to milestone and to div2, adding toc.'"/>
...
<!-- Added @target(s) to ref(s) in index.   -->
    ...
    <xsl:template match="tei:front//tei:ref">
        <xsl:copy>
            ...
            <xsl:attribute name="target" select="concat('#dec',$ref)"/>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>

<!-- div/@xml:id -->
<xsl:template match="tei:body/tei:div1[1]/tei:div2">
         <xsl:copy>
<xsl:copy-of select="@*"/>
             <xsl:variable name="div2" select="count(preceding::tei:div2)+1"/>
         <xsl:attribute name="xml:id" select="concat('dec',$div2)"/>
             <xsl:apply-templates/>
         </xsl:copy>
    </xsl:template>
