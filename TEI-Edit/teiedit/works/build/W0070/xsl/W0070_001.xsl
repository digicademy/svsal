...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0070_change_003'"/>
    <xsl:param name="editingDesc" as="xs:string" select="'Added @type to div2(s), list(s). @unit, @xml:id(s) to milestone(s)'"/>
    ...
    <xsl:template match="tei:div1[@type eq 'dict']//tei:list[not(@type)]">
        <xsl:copy>
        <xsl:copy-of select="@*"/>
    ...
    <xsl:template match="tei:div1[@type eq 'epilogue']//tei:div2[not(@type)]">
        <xsl:copy>
        <xsl:copy-of select="@*"/>
