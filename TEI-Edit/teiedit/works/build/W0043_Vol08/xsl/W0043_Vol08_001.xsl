...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2024-01-31'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0043_Vol08_change_007'"/>
...
<!--Added @n in div4. Some cases normalized in []. @type, doubt. @type section was added manually-->  
    <xsl:template match="tei:div4[not(@type) and ancestor::tei:div1[@xml:id = ('b47')]]" mode="second-round">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
    ...
    <xsl:template match="tei:div4[not(@type) and ancestor::tei:div1[@xml:id = ('b48','b49')]]" mode="second-round">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="nDiv" select="count(preceding::tei:div4[not(@type) and ancestor::tei:div2[@type eq 'segment'] = current()/ancestor::tei:div2[@type eq 'segment']])+1"/>
            <xsl:attribute name="n">
...
<!--Third round  mode="third-round"-->
    
    <xsl:variable name="third-round">
        <xsl:apply-templates select="$second-round" mode="third-round"/>
    </xsl:variable>
    <xsl:template match="@*|node()" mode="third-round">
        <xsl:copy>
            <xsl:apply-templates select="@*|node()" mode="third-round"/>
        </xsl:copy>
    </xsl:template>
<!--Delete targets from ref in toc for books 50, 51, 52, and 53. 
They appear in the following volume W0043_Vol09-->
    <xsl:template match="tei:ref[ancestor::tei:list[@xml:id =('list-b50','list-b51','list-b52','list-b53')]]" mode="third-round">
        <xsl:copy>
            <xsl:apply-templates mode="third-round"/>
        </xsl:copy>
    </xsl:template>
    ...
    <xsl:variable name="out">
        <xsl:copy-of select="$third-round"/>
    </xsl:variable>
