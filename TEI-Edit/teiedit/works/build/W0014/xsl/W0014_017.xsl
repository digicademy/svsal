...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0014_change_034'"></xsl:param>
    ...
    <xsl:function name="sal:mkLbId" as="xs:string">
        ...
        <xsl:variable name="lbId" as="xs:string">
            <xsl:choose>
                ...
                <!-- within marginal notes or labels, line numbering needs to take into account "virtual" pb elements that merely refer to real pb (using @sameAs). 
                     Thus, we also take into account pb[@sameAs] (unlike in other cases). -->
                <xsl:when test="$lbNode/ancestor::tei:note[@place='margin'] or $lbNode/ancestor::tei:label[@place='margin']"> 
                    ...
                    <xsl:variable name="currentMarginalLineOnPage" as="xs:integer">
                        <xsl:value-of select="count($lbNode/preceding::tei:lb[(not(@sameAs)
                                                                               ...
                                                                               and (./ancestor::tei:note[@place='margin'] or ./ancestor::tei:label[@place='margin'])]) + 1"/>
                    </xsl:variable>
                    <xsl:variable name="facs" as="xs:string" select="$lbNode/preceding::tei:pb[not(@sameAs)][1]/@facs"/>
                    <xsl:variable name="pageN" select="substring($facs, string-length($facs) - 3, 4)"/>
                        ...
                        <xsl:choose>
                            ...
                            <xsl:when test="$currentColumnOnPage > 0">
                                ...
                                <xsl:value-of select="count($lbNode/preceding::tei:lb[not(@sameAs) 
                                                                                      ...
                                                                                      and not(./ancestor::tei:note[@place='margin'] or ./ancestor::tei:label[@place='margin'])]) + 1"/>
                            </xsl:when>
