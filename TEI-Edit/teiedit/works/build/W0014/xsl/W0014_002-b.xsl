...
<!--    <xsl:output method="xml"/>-->
    ...
    <xsl:template match="tei:pb">
        <xsl:copy>
            ...
            <xsl:copy-of select="@resp|@cert|@change|@n|@sameAs"/>
    ...
    <xsl:template match="t:colShift[@cols]">
        <xsl:message terminate="yes" select="'Error: handling of t:colShift not yet specified'"/>
        <!--<xsl:choose>
            <xsl:when test="normalize-space(@cols) eq '' or xs:integer(@cols) lt 1">
    ...
    <xsl:template match="tei:hi">
        <xsl:choose>
            ...
            <xsl:when test="@rend = ('init', 'right', 'centered', 'spaced')">
                <xsl:choose>
                    <!-- a) omit rend='centered' in head or label[@type eq 'inline'] -->
                    <xsl:when test="(@rend = ('centered', 'right')) and (ancestor::tei:head or ancestor::tei:label[@type eq 'inline'])">
                        <xsl:apply-templates/>
                    </xsl:when>
    ...
    <xsl:template match="text()" priority="2">
        ...
        <xsl:choose>
            ...
            <xsl:when test="some $char in $quotationMarks satisfies contains(., $char)">
                <xsl:message terminate="yes" select="concat('Warning: quotation mark detected -- resolve this using element q: ', .)"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="."/>
            </xsl:otherwise>
