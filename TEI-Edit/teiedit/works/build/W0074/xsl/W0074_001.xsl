...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2022-12-28'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0074_change_003'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Tagged @n, @type in div(s) and list(s).'"/>
    ...
    <!--@type list(s).-->
    <xsl:template match="tei:list[ancestor::tei:div1[@type = 'index']]">
        <xsl:copy>
        <xsl:copy-of select="@*"/>
    ...
    <xsl:template match="tei:body//tei:div1">
        <xsl:copy>
        <xsl:copy-of select="@*"/>
            <xsl:variable name="n" select="count(preceding-sibling::tei:div1[ancestor::tei:body])+1"/>
    ...
    <xsl:template match="tei:div2[not(@type)]">
        <xsl:copy>
        <xsl:choose>
             <!--div2(s) @type question and @n-->
            <xsl:when test="ancestor::tei:div1[@xml:id = ('tract1','tract3')]">
                <xsl:attribute name="n" select="count(preceding-sibling::tei:div2[not(@type) and ancestor::tei:div1[@xml:id = ('tract1','tract3')]])+1"/>
