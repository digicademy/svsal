...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2024-09-05'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0074_change_004'"/>
...
<!-- 
    ...
    * Every template has a case and a mode:
    ...
    exception Final-ã pctã => peccatam mode="exception-peccatam"
...
<!--exception Final-ã pctã => peccatam mode="exception-peccatam"-->
    ...
    <xsl:variable name="exception-peccatam">
        <xsl:apply-templates select="$final-um" mode="exception-peccatam"/>
    </xsl:variable>
        
    <!-- identity transforms -->
    <xsl:template match="@*|node()" mode="exception-peccatam">
        <xsl:copy>
            <xsl:apply-templates select="@*|node()" mode="exception-peccatam"/>
        </xsl:copy>
    </xsl:template>
    
    <xsl:template match="text()[not(ancestor::tei:abbr or ancestor::*[@xml:lang = ('es','grc','gr','he','fr','pt','it')])]" mode="exception-peccatam">
        <xsl:analyze-string select="." regex="{'(\s)(pct)(ã|ã|ā|ā)([, \.:;\?!\)]+)'}">
            <xsl:matching-substring>
                <xsl:value-of select="regex-group(1)"/>
                <xsl:element name="abbr">
                    <xsl:attribute name="rend" select="'choice'"/>
...
<!-- 2) Final ā|ã - am, primā, verā"-->
    ...
    <xsl:variable name="Alexan">
        <xsl:apply-templates select="$exception-peccatam" mode="Alexan"/>
    </xsl:variable>
...
<!-- 8) ũ + t|tur, deducũtur deducuntur, mode="untur" -->  
    ...
    <xsl:variable name="untur">
        <xsl:apply-templates select="$exception-autem" mode="untur"/>
    </xsl:variable>
...
<!--17) final t with tilde - deprauet̃, = deprauetur, teneat̃ = teneatur mode="final-t-tilde"-->
    ...
    <xsl:template match="text()[not(ancestor::tei:abbr or ancestor::*[@xml:lang = ('la','grc','gr','he','fr','pt','it')])]" mode="final-t-tilde">
    ...
    <xsl:variable name="out">
        <xsl:copy-of select="$final-t-tilde"/>
    </xsl:variable>
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="Abbr" as="node()*" select="$final-t-tilde//tei:abbr[@rend eq 'abbr' and following-sibling::node()/self::tei:abbr[@rend eq 'expan' and matches(.,'[̃ ãāēẽõōũūꝓđ́]+')]]"/>
        <xsl:variable name="WrongExpansions" as="node()*" select="$final-t-tilde//tei:abbr[@rend eq 'expan' and matches(.,'[̃ ãāēẽõōũūꝓđ́]+')]"/>
        <xsl:choose>
            ...
            <!--Abbr with no special character, check this out.-->
            <xsl:when test="$final-t-tilde//tei:abbr[@rend eq 'abbr' and not(matches(.,'[ẽãāāēẽõōũūꝓꝰ́q́đ]+'))]">
            ...
            <xsl:otherwise>
                ...
                <xsl:variable name="Expansions" as="xs:integer" select="count($final-t-tilde//tei:abbr[@rend eq 'choice']//tei:abbr[@rend eq 'expan'])"/>
