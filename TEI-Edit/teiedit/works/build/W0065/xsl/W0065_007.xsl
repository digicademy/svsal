...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0019_change_07'"/>
...
<!--    
    ...
    <xsl:function name="local:hyphens" as="xs:boolean">
        ...
        <xsl:value-of select="not($lb/ancestor::tei:note) and $lb/preceding-sibling::*[1]/self::tei:hi[matches((.//text())[last()],'-$')]"/>
    </xsl:function>
    
    <xsl:template match="text()[matches(.,'-\s*$') and ./parent::tei:hi/following-sibling::*[1]/self::tei:lb[local:hyphens(.)]]" mode="round1">
