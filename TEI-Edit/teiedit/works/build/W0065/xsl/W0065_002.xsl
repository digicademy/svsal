...
<xsl:apply-templates mode="round-1"/>
    ...
    <xsl:template match="tei:back/tei:list" mode="round-1">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
        ...
        <xsl:copy>
            ...
            <xsl:choose>
                <xsl:when test="contains(@n, '.')">
                    <xsl:attribute name="n" select="substring-before(@n/string(), '.')"/>
                </xsl:when>
                <xsl:otherwise>
   ...
   <xsl:template match="tei:back//tei:item" mode="round-1">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
 ...
 <xsl:template match="tei:body//tei:note" mode="round-1">
<!-- TO DO : convert //note/text()[matches(.,'^\d+[\.:]?$') or matches(.,'§.? \d+[\.:]?$')] to milestones !-->
        <xsl:copy>
            <xsl:copy-of select="@*"/>   
            <xsl:choose>
    ...
    <xsl:choose>
                <xsl:when test="not(@rend)">
                    <xsl:attribute name="rend" select="'noRef'"/>
    ...
    <xsl:template match="//tei:div2/tei:div3" mode="round-1">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
