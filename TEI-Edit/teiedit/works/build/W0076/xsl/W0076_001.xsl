...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2022-05-03'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0076_change_004'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Added @type, @n to div2, div3.'"/>
    ...
    <xsl:template match="tei:div2[not(@xml:id)]">
    <xsl:copy>
    <xsl:copy-of select="@*"/>
        <xsl:variable name="n" select="count(preceding-sibling::tei:div2[not(@xml:id)])+1"/>
    ...
    <xsl:template match="tei:div3[not(@type)]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="n" select="count(preceding-sibling::tei:div3[not(@type) and ancestor::tei:div2 = current()/ancestor::tei:div2])+1"/>
            <xsl:attribute name="n" select="$n"/>
            <xsl:attribute name="type" select="'section'"/>
            <xsl:apply-templates/>
        </xsl:copy>
