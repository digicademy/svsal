...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0076_change_005'"/>
...
<!--30) ⁊ (char204a) ==> y , mode="only-y"-->
    ...
    <xsl:variable name="only-que">
        <xsl:apply-templates select="$only-y" mode="only-que"/>
    </xsl:variable>
...
<!-- ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ -->
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="Abbr" as="node()*" select="$only-que//tei:abbr[@rend eq 'abbr' and following-sibling::node()/self::tei:abbr[@rend eq 'expan' and matches(.,'[̃ ãāēẽõōũūꝓđ]+')]]"/>
        <xsl:variable name="WrongExpansions" as="node()*" select="$only-que//tei:abbr[@rend eq 'choice']//tei:abbr[@rend eq 'expan' and matches(.,'[̃ ãāēẽõōũūꝓđ]+')]"/>
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:variable name="Expansions" as="xs:integer" select="count($only-que//tei:abbr[@rend eq 'choice']//tei:abbr[@rend eq 'expan'])"/>
