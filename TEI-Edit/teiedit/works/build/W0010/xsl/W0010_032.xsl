...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2024-04-25'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0010_change_053'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Added xml:id(s) to quote(s).'"/>
    ...
    <xsl:copy-of select="@*"/>
        ...
        <xsl:variable name="facs" select="preceding::tei:lb[1]/@xml:id"/>
        <xsl:value-of select="replace($facs,'lb','qu')"/>
        </xsl:attribute>
        <xsl:apply-templates/>
    </xsl:copy>
    </xsl:template>
