...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- !! PARAMETERS (to be adjusted / injected as arguments) !! -->
    ...
    <xsl:param name="specialCharFile" as="xs:string" select="'../../../resources/chars/specialchars_2019-02-27.xml'"/>
    
    <!-- if delimitMode is false, newline (\n) and certain elements (see $delimiterElems) will be included 
            as word delimiters (not only the "usual" interpunctuation) -> this may go wrong when \n does not signify an actual blank character! -->
    <xsl:param name="delimitMode" as="xs:boolean" select="true()"/>
    
    <!-- The usual stuff: -->
    <xsl:param name="editors" as="xs:string" select="'#DG #auto'"/>
    <xsl:param name="editingDate" as="xs:string" select="'2019-03-04'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0010_change_030'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="concat('Automatically expanded abbreviations (', $lang,').')"/>
    ...
    <!-- general TODOs: 
        ...
        - expand abbreviations with arbitrary stem (e.g. - *chare8bf (* = Wort) -> *que) : add wildcard option to abbrPart[@type eq 'text']
    ...
    <xsl:variable name="wordDelimiters" as="xs:string">
        <xsl:choose>
            <xsl:when test="$delimitMode"><xsl:value-of select="'[, \?!]'"/></xsl:when> <!-- assuming here that blank always signifies a word boundary -->
            <xsl:otherwise><xsl:value-of select="'[,\s\?!]'"/></xsl:otherwise> <!-- any whitespace (incl. \n) is allowed -->
        </xsl:choose>
    </xsl:variable> 
    <xsl:variable name="delimiterElems" as="xs:string+" select="('lb', 'cb', 'pb', 'milestone', 'ref', 'note')"/>
    ...
    <!-- Determines whether a node is the beginning of an abbreviation. If this is the case, a tuple containing the following strings 
        ...
        - 2) the reach of the abbreviation, expressed in a number stating how many following siblings of the analyzed nodes are part of the abbreviation token
        ...
        <xsl:choose>
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:variable name="abbrCandidates" as="element(local:abbrPart)*" 
                            select="$abbrExpanList/local:abbrExpan/local:abbreviation/local:abbrPart[1][@type eq 'text' and matches($node, concat($wordDelimiters, ./text(), '$'))]"/>
            ...
            <xsl:when test="$node/self::tei:g">
                <xsl:choose>
                    ...
                    <xsl:when test="local:isDelimitedLeft($node) and local:isDelimitedRight($node) 
    ...
    <xsl:function name="local:compareAbbrNodes" as="xs:integer">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$node1/@type eq 'g'">
                <xsl:if test="not(local:isValidCharRef($node1/text()))"><xsl:message terminate="yes" select="concat('ERROR: invalid char reference: ', $node1/text())"/></xsl:if>
                <xsl:choose>
                    <xsl:when test="$mode eq 'last'">
                        <xsl:choose>
    ...
    <xsl:function name="local:isDelimitedLeft" as="xs:boolean">
        ...
        <xsl:choose>
            <xsl:when test="$g/preceding-sibling::node()[1]/self::text()[matches(., concat($wordDelimiters, '$'))]
                            or (not($delimitMode) and $g/preceding-sibling::node()[1]/self::*[local-name(.) = $delimiterElems])">
                <xsl:value-of select="true()"/>
            </xsl:when>
            <xsl:otherwise><xsl:value-of select="false()"/></xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:function name="local:isDelimitedRight" as="xs:boolean">
        ...
        <xsl:choose>
            <xsl:when test="$g/following-sibling::node()[1]/self::text()[matches(., concat('^', $wordDelimiters))]
                            or (not($delimitMode) and $g/following-sibling::node()[1]/self::*[local-name(.) = $delimiterElems])">
                <xsl:value-of select="true()"/>
            </xsl:when>
            <xsl:otherwise><xsl:value-of select="false()"/></xsl:otherwise>
        </xsl:choose>
