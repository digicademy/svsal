...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDesc" as="xs:string" select="'Annotated marked hyphenations.'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0010-change-019'"/>
    ...
    <xsl:function name="local:isHyphenation" as="xs:boolean">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$elem/preceding-sibling::node()[not(self::text() and normalize-space() eq '')][1][self::tei:note[@type eq 'margin'] or self::tei:label[@type eq 'margin'] or self::tei:milestone]">
            ...
            <!-- 3.) hyphen occurs in the context of hi -->
            <!-- a) hyphen-ending text occurs within tei:hi (the latter being followed by the break element) -->
            <xsl:when test="$elem/preceding-sibling::node()[not(self::text() and normalize-space() eq '')][1]/self::tei:hi[matches(descendant::text()[last()], '[-=]$')]">
                <xsl:value-of select="true()"/>
            </xsl:when>
            <!-- b) TODO: break occurs within tei:hi, but hyphen-ending text does not -->
            <!-- c) both the break and the hyphen-ending text each occur within tei:hi -->
    ...
    <xsl:function name="local:removeHyphen">
        <xsl:param name="textNode" as="text()"/>
            <xsl:choose>
                ...
                <!-- TODO: include label[@type eq 'margin'] in here -->
                <xsl:when test="matches($textNode, '[-=]\n?$') 
                                and $textNode/following-sibling::node()[1][self::tei:note[@type eq 'margin'] or self::tei:label[@type eq 'margin'] or self::tei:milestone]
    ...
    <xsl:function name="local:isInterferingWhitespace" as="xs:boolean">
        ...
        <xsl:choose>
            <!-- whitespace between a hyphenation break element and some other element (currently, we only check for tei:hi here) -->
