...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="tei:lb">
        ...
        <xsl:if test="@rend eq 'hyphenation'">
    ...
    <xsl:template match="tei:cb">
        <xsl:copy>
            <xsl:if test="@n">
                <xsl:message select="'Found cb/@n: refers to actual column number?'"/>
            </xsl:if>
            <xsl:copy-of select="@type|@n|@resp|@cert|@change|@sameAs"/>
            <xsl:if test="preceding-sibling::node()[not(self::text() and normalize-space() eq '')][1]/self::t:colShift[xs:integer(@cols) gt 1]">
                <xsl:attribute name="type" select="'start'"/>
                <xsl:message select="'Tagging tei:cb as beginning of multi-column layout...'"/>
            </xsl:if>
            <xsl:if test="@xml:id">
                <xsl:attribute name="xml:id" select="generate-id(.)"/>
            </xsl:if>
    ...
    <xsl:template match="t:colShift">
        <xsl:message select="'Found tite:colShift not yet resolved...'"/>
        <xsl:choose>
            <!-- if colShift marks beginning of a multi-column layout, delete it altogether, but only if it has a matching cb -->
            <xsl:when test="xs:integer(@cols) gt 1">
                <xsl:if test="not(following-sibling::node()[not(self::text() and normalize-space() eq '')][1]/self::tei:cb)">
                    <xsl:message terminate="yes" select="'Found tite:colShift marking the beginning of a multi-column layout, but there seems to be no matching cb.'"/>
                </xsl:if>
            </xsl:when>
            <!-- if colShift marks end of multi-column layout, transform it to cb[@type eq 'end'] -->
            <xsl:when test="xs:integer(@cols) eq 1">
                <xsl:element name="cb">
            ...
            <xsl:otherwise>
                <xsl:message terminate="yes" select="'tite:colShift/@cols has invalid value or is lacking.'"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="tei:note[@type='marginalia']">
        <xsl:copy>
            ...
            <xsl:copy-of select="@resp|@cert|@change|@xml:lang|@anchored"/>
            <xsl:attribute name="xml:id" select="generate-id(.)"/>
    ...
    <xsl:template match="tei:hi">
        <xsl:choose>
            ...
            <xsl:when test="@rend = ('init', 'right', 'centered', 'spaced', 'centered spaced', 'spaced italics')">
                <xsl:copy>
    ...
    <xsl:template match="*[not(self::tei:hi)][@rend or @rendition]">
    ...
    <xsl:template match="tei:seg[@type eq 'gap' and @rend eq 'gap']">
    ...
    <xsl:template match="comment()" priority="2">
        <xsl:choose>
            <xsl:when test="matches(normalize-space(.), '^tao:A[-\+][1-4]_(beg|end)$')"/> <!-- delete font size information -->
