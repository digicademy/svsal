...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2021-10-05'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0010_change_051'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Correction of notes: adding @anchored and @type and @n to ref-elements.'"/>
    ...
    <xsl:template match="tei:text//tei:note[preceding-sibling::*[1]/self::tei:ref]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
    ...
    <xsl:template match="tei:ref[following-sibling::*[1]/self::tei:note]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:attribute name="n" select="following-sibling::*[1]/self::tei:note/@n"/>
            <xsl:attribute name="type" select="'note-anchor'"/>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
    <xsl:template match="tei:text//tei:note[preceding-sibling::*[1]/self::tei:supplied]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:attribute name="anchored" select="'true'"/>
            <xsl:apply-templates/>
    ...
    <xsl:template match="tei:ref[ancestor::tei:supplied]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:attribute name="n" select="descendant::text()"/>
            <xsl:attribute name="type" select="'note-anchor'"/>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
