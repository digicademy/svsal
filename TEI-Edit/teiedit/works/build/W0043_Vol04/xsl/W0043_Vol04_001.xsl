...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2022-11-24'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0043_Vol04_change_007'"/>
...
<!--Added @n, @type, @xml:id in div2-->
    <xsl:template match="tei:div2[not(@type)]" mode="first-round">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="nDiv" select="count(preceding::tei:div2[not(@type)][ancestor::tei:div1[@type eq 'book'] = current()/ancestor::tei:div1[@type eq 'book']][not(@type)])+1"/>
...
<!--Added @n, @type, @xml:id in div4 Liber XXIV-->
    <xsl:template match="tei:div4[ancestor::tei:div1[@xml:id eq 'b24']]" mode="first-round">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="nDiv" select="count(preceding::tei:div4[not(@type)][ancestor::tei:div1[@xml:id eq 'b24'] = current()/ancestor::tei:div1[@xml:id eq 'b24']])+1"/>
            <xsl:choose>
            <xsl:when test="not(@n)">
                <xsl:attribute name="n" select="$nDiv"/>
            </xsl:when>
...
<!--Added normalized @n, @type, @xml:id in div4 Liber XXV-->
    <xsl:template match="tei:div4[ancestor::tei:div1[@xml:id eq 'b25']]" mode="first-round">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="nDiv" select="count(preceding::tei:div4[not(@type)][ancestor::tei:div1[@xml:id eq 'b25'] = current()/ancestor::tei:div1[@xml:id eq 'b25']])+1"/>
            <xsl:choose>
            <xsl:when test="not(@n)">
...
<!--Added normalized @n, @type, @xml:id in div4 Liber XXVI-->
    <xsl:template match="tei:div4[ancestor::tei:div1[@xml:id eq 'b26']]" mode="first-round">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="nDiv" select="count(preceding::tei:div4[not(@type)][ancestor::tei:div1[@xml:id eq 'b26'] = current()/ancestor::tei:div1[@xml:id eq 'b26']])+1"/>
            <xsl:choose>
            <xsl:when test="not(@n)">
                <xsl:attribute name="n" select="concat('[',$nDiv,']')"/>
            </xsl:when>
...
<!--Added @n, @type, @xml:id in div4 Liber XXVII-->
    <xsl:template match="tei:div4[ancestor::tei:div1[@xml:id eq 'b27']]" mode="first-round">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="nDiv" select="count(preceding::tei:div4[not(@type)][ancestor::tei:div1[@xml:id eq 'b27'] = current()/ancestor::tei:div1[@xml:id eq 'b27']])+1"/>
            <xsl:choose>
            <xsl:when test="not(@n)">
                <xsl:attribute name="n" select="$nDiv"/>
            </xsl:when>
...
<!-- div2 "Segmentlevel"-->
    <xsl:template match="tei:ref[starts-with(.,'S')]" mode="first-round">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
...
<!-- div3 "Chapterlevel" -->
    ...
    <xsl:template match="tei:ref[not(@target)]" mode="second-round">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="nCap" select="count(preceding::tei:ref[not(@target) and ancestor::tei:list[@xml:id] = current()//ancestor::tei:list[@xml:id]])+1"/>
            <xsl:variable name="book-id"  select="current()/ancestor::tei:list/@xml:id"/>
