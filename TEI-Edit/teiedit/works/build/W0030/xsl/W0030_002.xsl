...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDesc" as="xs:string" select="'Changed lb attributes.'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0030_change_004'"/>
    ...
    <xsl:template match="tei:lb[@type eq 'nb']">
        <xsl:copy>
        <xsl:variable name="string" select="preceding::text()[1]"/>
            ...
            <xsl:choose>
                ...
                <xsl:when test="not(ends-with($string,'-|='))">
                    <xsl:attribute name="break" select="'no'"/>
    ...
    <xsl:template match="text()[matches(.,'[-=]\n?$') and following-sibling::node()[1][(self::tei:pb or self::tei:cb or self::tei:lb or self::tei:note )]]">
        <xsl:choose>
