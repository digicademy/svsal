...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="tei:pb">
        <xsl:copy>
            <xsl:if test="(not(@n) or normalize-space(@n) eq '' or not(matches(@n, '\[?[\dxivjfol\.\s]{1,7}\]?[rv]?'))) and not(@sameAs)">
    ...
    <xsl:template match="tei:milestone[@rend eq 'hand']">
        <xsl:element name="figure">
            <xsl:attribute name="type" select="'hand'"/>
            <xsl:attribute name="place" select="'margin'"/>
        </xsl:element>
    </xsl:template>
