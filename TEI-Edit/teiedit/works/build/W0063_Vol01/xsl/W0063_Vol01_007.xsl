...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2024-01-24'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0063_Vol01_change_011'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Rearranged pb/@sameAs and lb/xml:id(s) in gloss.'"/>
    ...
    <!--Rearranged pb(s) in div[@type eq 'gloss'] as @sameAs. They were transformed in TEI-Transformation as @facs to nummber lines and add xml:id(s).
        See W0063_Vol01_002b.xsl lines: 361, 379-->
    
    <xsl:template match="tei:pb[ancestor::tei:div[@type eq 'gloss']]">
        <xsl:copy>
            <xsl:copy-of select="@* except @facs"/>
            <xsl:variable name="id" select="preceding::tei:pb[@facs = current()/@facs]/@xml:id"/>
            <xsl:attribute name="sameAs" select="concat('#',$id)"/>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
    
    <!--Added "-gl" at the end of lb/@xml:id(s) to give unique ids to lines in gloss.
        e.g.:
        W0063-01-0005-lb-0001 for first line in partida W0063-01-0005 (Spanish)
        W0063-01-0005-lb-0001-gl for first line in gloss W0063-01-0005 (Latin)  -->
    
    <xsl:template match="tei:lb[ancestor::tei:div[@type eq 'gloss']]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:attribute name="xml:id" select="concat(@xml:id, '-gl')"/>
            <xsl:apply-templates/>
        </xsl:copy>
     </xsl:template>
