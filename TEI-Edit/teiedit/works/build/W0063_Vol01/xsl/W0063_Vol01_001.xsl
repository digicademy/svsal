...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2023-11-14'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0063_Vol01_change_004'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Tagged @n, @type, @xml:id(s) in div(s), ref(s) and @target in TOC.'"/>
    ...
    <xsl:template match="@*|node()">
     ...
     <!--Added @n, @type, @xml:id in div2 partida 1 Maintext-->
    <xsl:template match="tei:div2[not(@type) and ancestor::tei:div1[@type eq 'partida']]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="nDiv" select="count(preceding::tei:div2[not(@type)][ancestor::tei:div1[@type eq 'partida'] = current()/ancestor::tei:div1[@type eq 'partida']])+1"/>
            <xsl:attribute name="xml:id" select="concat('titel-',$nDiv)"/>
    ...
    <!--Added TOC references-->
    <xsl:template match="tei:ref[not(@target) and ancestor::tei:div1[@type eq 'contents']]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="title-id"  select="count(preceding::tei:ref[not(@target) and ancestor::tei:div1[@type eq 'contents']])+1"/>
            <xsl:attribute name="target" select="concat('#titel-',$title-id)"/>  
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
    <!--Added @n, @type, @xml:id in div2 Glossen-->
    <xsl:template match="tei:div2[not(@type) and ancestor::tei:div1[@type eq 'gloss']]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="nDiv" select="count(preceding::tei:div2[not(@type)][ancestor::tei:div1[@type eq 'gloss'] = current()/ancestor::tei:div1[@type eq 'gloss']])+1"/>
            <xsl:attribute name="xml:id" select="concat('gloss-titel-',$nDiv)"/>
            <xsl:attribute name="n">
            <xsl:choose>
                <xsl:when test="@n"><xsl:value-of select="@n"/></xsl:when>
                <xsl:otherwise><xsl:value-of select="$nDiv"/></xsl:otherwise>
            </xsl:choose>
            </xsl:attribute>
            <xsl:attribute name="type" select="'title'"/>  
            <xsl:apply-templates/>
    ...
    <!--Added @n, @type, in div3 leyes partida-->
    <xsl:template match="tei:div3[not(@type) and ancestor::tei:div1[@type eq 'partida']]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="nDiv" select="count(preceding::tei:div3[not(@type)][ancestor::tei:div2 = current()/ancestor::tei:div2])+1"/>
            <xsl:attribute name="n" select="$nDiv"/>
            <xsl:attribute name="type" select="'law'"/>  
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
    <!--Added @n, @type, in div3 lex gloss-->
    <xsl:template match="tei:div3[not(@type) and ancestor::tei:div1[@type eq 'gloss']]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="nDiv" select="count(preceding::tei:div3[not(@type)][ancestor::tei:div2 = current()/ancestor::tei:div2])+1"/>
            <xsl:attribute name="n">
    ...
    <!--Added @xml:id(s) in gloss references to make links back and forth-->
    <xsl:template match="tei:ref[ancestor::tei:div1[@type eq 'gloss']]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:attribute name="xml:id" select="parent::tei:p/@xml:id"/>  
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
    <!--Changed @xml:id(s) in gloss p(s)-->
    <xsl:template match="tei:p[ancestor::tei:div1[@type eq 'gloss']]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:attribute name="xml:id" select="replace(current()/@xml:id,'gloss','p-gloss')"/>  
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
