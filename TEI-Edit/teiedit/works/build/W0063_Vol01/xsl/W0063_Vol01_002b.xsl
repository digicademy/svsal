...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="tei:ref[ancestor::tei:div1[@type eq 'preface']]">
        <xsl:copy>
            <xsl:copy-of select="@resp|@cert|@change|@n|@xml:lang|@type"/>
            <xsl:attribute name="xml:id" select="generate-id(.)"/>
            <xsl:if test="@target">
    ...
    <!--Added to fit with primera partida CR 24.01.24--> 
    <xsl:template match="tei:ref[ancestor::tei:div1[@type eq 'partida']]">
        <xsl:copy>
            <xsl:copy-of select="@resp|@cert|@change|@n|@xml:lang|@type"/>
            <xsl:attribute name="xml:id" select="generate-id(.)"/>
            <xsl:if test="@target">
    ...
    <!--<xsl:template match="tei:ref[not(ancestor::tei:div1[@type eq 'contents'])]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:apply-templates/>
        </xsl:copy>
    ...
    <xsl:template match="tei:p[not(@rend)]">
        <xsl:copy>
            <xsl:copy-of select="@corresp|@resp|@cert|@change|@n|@xml:lang"/>
            <xsl:attribute name="xml:id" select="generate-id(.)"/>
            <xsl:if test="@rend">
    ...
    <xsl:template match="tei:pb">
        <xsl:copy>
            ...
            <xsl:copy-of select="@resp|@cert|@change|@n|@type"/> <!-- Deleted |@sameAs line to implement xml:ids in W0063_Vol01_005.xsl and W0063_Vol01_006.xsl 24.01.24 CR -->            
...
<!--Changed @sameAs to implement xml:ids in W0063_Vol01_005.xsl and W0063_Vol01_006.xsl 24.01.24 CR -->                
                <xsl:when test="@sameAs">
                    <xsl:attribute name="facs" select="replace(current()/@sameAs,'#','facs:')"/>
