...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2021-03-02'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0061_change_028'"/>
        ...
        <xsl:choose>
            ...
            <xsl:when test="//tei:choice[./*[1]//(tei:pb|tei:cb|tei:lb) and ./*[2][.//tei:g and not(.//(tei:pb|tei:cb|tei:lb))]]">
                <xsl:message terminate="yes" select="concat('WARN: found choice to be resolved, with special char (g) in second child - resolve this manually (in line ',
                                                     //tei:choice[child::*[1]//(tei:pb|tei:cb|tei:lb) and child::*[2][tei:g and not((tei:pb|tei:cb|tei:lb))]]/preceding::tei:lb[1]/@xml:id,').')"></xsl:message>
            </xsl:when>
