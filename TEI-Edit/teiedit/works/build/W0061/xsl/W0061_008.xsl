...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0061_change_011'"></xsl:param>
    ...
    <xsl:template match="tei:pb" mode="breakPB_CB">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:choose>
                <xsl:when test="following-sibling::node()[2]/self::tei:lb[@rendition eq '#hyphen']">
                    <xsl:attribute name="break" select="'no'"/>
                    <xsl:attribute name="rendition" select="'#hyphen'"/>
                </xsl:when>
                <xsl:when test="following-sibling::node()[2]/self::tei:lb[@rendition='#noHyphen']">
                    <xsl:attribute name="break" select="'no'"/>
                    <xsl:attribute name="rendition" select="'#noHyphen'"/>
                </xsl:when>
                <xsl:when test="following-sibling::node()[1]/self::tei:lb[@rendition eq '#hyphen']">
                    <xsl:attribute name="break" select="'no'"/>
                    <xsl:attribute name="rendition" select="'#hyphen'"/>
                </xsl:when>
                <xsl:when test="following-sibling::node()[1]/self::tei:lb[@rendition='#noHyphen']">
                    <xsl:attribute name="break" select="'no'"/>
                    <xsl:attribute name="rendition" select="'#noHyphen'"/>
                </xsl:when>
                <xsl:otherwise/>
