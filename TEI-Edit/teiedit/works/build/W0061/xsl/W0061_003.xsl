...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2018-09-10'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0061_change_09'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Adding ref @target for the note anchor.'"/>
    ...
    <xsl:template match="t:i[following-sibling::node()[1]/self::tei:note]">
        <xsl:element name="ref">
            <!--<xsl:message>Info: created ref element with @target, @type</xsl:message>-->
            <xsl:variable name="id" select="following-sibling::tei:note[1]/@xml:id"/>
            <xsl:attribute name="target" select="concat('#', $id)"/>
            <xsl:attribute name="type" select="'note_anchor'"/>
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:apply-templates/>
        </xsl:copy>
