...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2020-04-21'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0011_change_027'"/>
    ...
    <xsl:template match="tei:choice[(child::*[1]/tei:pb or child::*[1]/tei:cb or child::*[1]/tei:lb)
        ...
        <xsl:variable name="lb1" as="element(tei:lb)?" select="./child::*[1]/tei:lb"/><!--'Error: Resolve manually - more than one lb in choice/abbr: //tei:choice//abbr/lb[2]'-->
            ...
            <xsl:if test="$lb1">
                <xsl:element name="lb">
                    <xsl:if test="not($lb1/@break eq 'no') or not($lb1/@xml:id)">
                        <xsl:message terminate="no" select="'Error: element ', $lb1/@xml:id, ' is lacking @break or @xml:id'"/> <!-- if it lacks @xml:id, search for it manually... -->
                    </xsl:if>
                    <xsl:attribute name="rendition" select="'#noHyphen'"/>
                    <xsl:attribute name="break" select="'no'"/>
    ...
    <xsl:template match="/">
        ...
        <xsl:choose>
            ...
            <xsl:when test="//tei:choice[./*[1]//(tei:pb|tei:cb|tei:lb) and ./*[2][.//tei:g and not(.//(tei:pb|tei:cb|tei:lb))]]">
                <xsl:message terminate="no" select="concat('WARN: found choice to be resolved, with special char (g) in second child - resolve this manually (in line ',
