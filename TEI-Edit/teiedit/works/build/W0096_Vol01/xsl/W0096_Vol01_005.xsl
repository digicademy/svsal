...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2018-11-29'"/>
    <xsl:param name="editingDesc" as="xs:string" select="'Tagged special characters.'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0096_Vol01_change_011'"/>
    ...
    <xsl:template match="tei:text//text()" priority="2">
        ...
        <xsl:choose>
            ...
            <xsl:when test="(normalize-space(.) != '') and not(ancestor::tei:g or ancestor::tei:foreign[@xml:lang=('gr', 'gre', 'grc','he')])">
