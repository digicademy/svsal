...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2024-09-12'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0063_Vol02_change_010'"/>
    ...
    <!--Added to fit with primera partida CR 24.01.24--> 
    ...
    <!--Changed to fit with primera partida CR 24.01.24--> 
    <xsl:template match="tei:text//tei:ref">
        <xsl:copy>
            ...
            <!--Added "reft, ref" to fit with primera partida CR 24.01.24-->
            <xsl:variable name="reft" select="key('ref', substring-after(@target, '#'))"/>
            <xsl:variable name="ref" select="key('ref', @xml:id)"/>
            <xsl:choose>
                <xsl:when test="$milestone or $p or $div or $list or $item or $mnote or $pb or $ref or $reft">
                    <xsl:choose>
                        ...
                        <!--Added to fit with primera partida CR 24.01.24--> 
                        <xsl:when test="$ref"> 
                            <xsl:attribute name="target" select="concat('#', sal:mkContainerElemId($reft, 're'))"/>
                            <xsl:attribute name="xml:id" select="sal:mkContainerElemId($ref, 're')"/>
                        </xsl:when>                           
    ...
    <!--Not used to fit with primera partida CR 24.01.24. since there are not relevant marginal notes.--> 
