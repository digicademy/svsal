...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2021-04-30'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0083_change_017'"/>
    ...
    <xsl:template match="/">
        ...
        <xsl:if test="count($choiceDuplicatesMain) gt 0">
            ...
            <xsl:message terminate="no" select="concat('ERROR: found ', count($choiceDuplicatesMain), ' cases of adjacent choice with identically resolved text in the main area.')"/>
        </xsl:if>
...
<xsl:function name="local:choiceHasNoBorder" as="xs:boolean">
        ...
        <xsl:choose>
            <xsl:when test="$precSibling/self::tei:choice//text()[not(matches(., '[\.,\s:;\?!\(\)]$'))]
                            or $follSibling/self::tei:choice//tei:corr//text()[not(matches(., '^[\.,\s:;\?!\(\)]'))]
