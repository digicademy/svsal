...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2020-10-21'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0083_change_012'"></xsl:param>
    ...
    <!-- expanding abbreviations of spanish words depending on their Endings e.g.:
        ...
        "ento - mandamiẽto", "encia - differẽcia"
    ...
    <xsl:template match="tei:text//text()[not(ancestor::tei:abbr)]" mode="endo">
        <xsl:analyze-string select="." regex="{'(\s)([a-zſç]+)(ẽ|ē)(do)([\s\.,;\(\)])'}">
    ...
    <xsl:template match="tei:text//text()[not(ancestor::tei:abbr)]" mode="ando">
        <xsl:analyze-string select="." regex="{'(\s)([aA-zZſçñ]+)(ã|ā)(do)([\s\.,;\(\)])'}">
            <xsl:matching-substring>
                <xsl:value-of select="regex-group(1)"/>
                <xsl:element name="abbr">
                    <xsl:attribute name="rend" select="'choice'"/>
    ...
    <xsl:template match="tei:text//text()[not(ancestor::tei:abbr)]" mode="mente">
        <xsl:analyze-string select="." regex="{'(\s)([aA-zZſçñ]+)(ẽ|ē)(te)([\s\.,;\(\)])'}">
            <xsl:matching-substring>
                <xsl:value-of select="regex-group(1)"/>
                <xsl:element name="abbr">
                    <xsl:attribute name="rend" select="'choice'"/>
    ...
    <xsl:template match="tei:text//text()[not(ancestor::tei:abbr)]" mode="ende">
        <xsl:analyze-string select="." regex="{'(\s)([aA-zZſçñ]+)(ẽ|ē)(de)([\s\.,;\(\)])'}">
            <xsl:matching-substring>
                <xsl:value-of select="regex-group(1)"/>
                <xsl:element name="abbr">
                    <xsl:attribute name="rend" select="'choice'"/>
    ...
    <xsl:template match="tei:text//text()[not(ancestor::tei:abbr)]" mode="cion">
        <xsl:analyze-string select="." regex="{'(\s)([aA-zZſçñ]+)(zi|ſi|si|ci)(õ)([\s\.,;\(\)])'}">
            <xsl:matching-substring>
                <xsl:value-of select="regex-group(1)"/>
                <xsl:element name="abbr">
                    <xsl:attribute name="rend" select="'choice'"/>
    ...
    <xsl:template match="tei:text//text()[not(ancestor::tei:abbr)]" mode="ento">
        <xsl:analyze-string select="." regex="{'(\s)([aA-zZſçñ]+)(ẽ|ē)(to)([\s\.,;\(\)])'}"><!--(\w+[^ãẽõũ])-->
            <xsl:matching-substring>
                <xsl:value-of select="regex-group(1)"/>
                <xsl:element name="abbr">
                    <xsl:attribute name="rend" select="'choice'"/>
    ...
    <xsl:template match="tei:text//text()[not(ancestor::tei:abbr)]" mode="encia">
        <xsl:analyze-string select="." regex="{'(\s)([aA-zZſçñ]+)(ẽ|ē)(cia)([\s\.,;\(\)])'}"><!--(\w+[^ãẽõũ])-->
            <xsl:matching-substring>
                <xsl:value-of select="regex-group(1)"/>
                <xsl:element name="abbr">
                    <xsl:attribute name="rend" select="'choice'"/>
    ...
    <xsl:template match="tei:text//text()[not(ancestor::tei:abbr)]" mode="ancia">
        <xsl:analyze-string select="." regex="{'(\s)([aA-zZſçñ]+)(ã|ā)(cia)([\s\.,;\(\)])'}"><!--(\w+[^ãẽõũ])-->
            <xsl:matching-substring>
                <xsl:value-of select="regex-group(1)"/>
                <xsl:element name="abbr">
                    <xsl:attribute name="rend" select="'choice'"/>
...
<!-- ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ -->
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="Expansions" as="xs:integer" select="count($ancia//tei:abbr[@rend eq 'choice']//tei:abbr[@rend eq 'expan'])"/>
