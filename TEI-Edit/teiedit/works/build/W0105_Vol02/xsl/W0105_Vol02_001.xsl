...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:local="http://salamanca.adwmainz.de" xmlns="http://www.tei-c.org/ns/1.0" exclude-result-prefixes="xs" xmlns:t="http://www.tei-c.org/ns/tite/1.0" version="2.0">
    ...
    <xsl:template match="tei:body//tei:div1[@type eq 'quaestio' ]" mode="round-1">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="nDiv" select="count(preceding::tei:div1[@type eq 'quaestio' and ancestor::tei:body]) + 1"/>
<xsl:variable name="nDiv40" select="//tei:body/tei:div1[@type eq 'quaestio' ][1]/@n"/>

<xsl:choose>
<xsl:when test="@n">
<xsl:attribute name="n" select="@n"/></xsl:when>
         <xsl:otherwise> <xsl:attribute name="n" select="$nDiv40 + $nDiv -1"/></xsl:otherwise>
</xsl:choose>
            <xsl:attribute name="type" select="'question'"/>
            <xsl:attribute name="xml:id" select="concat('quest', $nDiv40 + $nDiv -1)"/>
...
<xsl:choose>
    <xsl:when test="(contains(., 'Dispvt') or contains(., 'DISPVT')) "> 
<!-- Disputationes are counted from the body, not the div1 parent ! -->
<xsl:variable name="nDisp" select="count(preceding::tei:div2[(contains(., 'Dispvt') or contains(., 'DISPVT')) and not(@type eq 'section') and ancestor::tei:body]) + 1"/>
<xsl:variable name="nDisp150" select="//tei:div1/tei:div2[contains(., 'Dispvt') and not(@type eq 'section')][1]/@n"/>

<xsl:choose>
    <xsl:when test="@n">
<xsl:attribute name="xml:id" select="concat('dispute', replace(@n/string(), '(\[*)(\d+)(.*)', '$2'))"/>
</xsl:when>
<xsl:otherwise>
<xsl:variable name="nDisp" select="count(preceding::tei:div2[(contains(., 'Dispvt') or contains(., 'DISPVT'))  and not(@type eq 'section') and ancestor::tei:body]) + 1"/>
  <xsl:attribute name="n" select="$nDisp150 + $nDisp -1"/>
<xsl:attribute name="xml:id" select="concat('dispute', $nDisp150 + $nDisp -1)"/>
</xsl:otherwise>
</xsl:choose>
 ...
 <xsl:attribute name="type" select="'disputation'"/>
  ...
  <xsl:when test="(contains(., 'Articvl') or contains(., 'ARTICVL'))"> 
<xsl:variable name="nArt" select="count(preceding-sibling::tei:div2[(contains(., 'Articvl') or contains(., 'ARTICVL')) and not(@type eq 'section') and ancestor-or-self::tei:div1]) + 1"/>
<xsl:choose>
<xsl:when test="not(./@n)">
...
<xsl:template match="tei:list[@type='contents']/tei:item/tei:list/tei:head//tei:ref" mode="round-3">
   <xsl:copy>
            <xsl:copy-of select="@*"/>
<xsl:variable name="DispOrig" select="150"/>
<xsl:variable name="nDisp" select=" $DispOrig + count(preceding::tei:head//tei:ref)+1"/>
<xsl:attribute name ="xml:id" select="concat('ref_dispute', $nDisp -1)"/>
<xsl:attribute name="target" select="concat('#dispute', $nDisp -1)"/>
     <xsl:apply-templates mode="round-3"/>
   </xsl:copy>
</xsl:template>
...
<xsl:variable name="divID" select="substring-after(../../tei:head//tei:ref/@target, '#')"/>
                   ...
                   <xsl:variable name="refNumber" select="count(../preceding-sibling::tei:item[ancestor::tei:list]) +1"/> 
<xsl:attribute name="n" select="$refNumber"/>
...
<xsl:template match="tei:back//tei:div1[@type eq 'index']/tei:list" mode="round-4">
<xsl:copy>
<xsl:copy-of select="@*"/>
