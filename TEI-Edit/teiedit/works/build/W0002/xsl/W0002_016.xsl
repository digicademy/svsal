...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- SPANISH VERSION of single-character abbreviation expansion -->
    
    <xsl:output method="xml"/> 
    
    <xsl:param name="editors" as="xs:string" select="'#DG #auto'"/>
    <xsl:param name="editingDate" as="xs:string" select="'2018-11-08'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0002_change_020'"></xsl:param>
    ...
    <!-- define SPANISH single-character abbreviations and their expansion -->
    <xsl:variable name="expansions">
        <local:expansions>
            <local:exp id="charq0303">que</local:exp> <!-- LATIN SMALL LETTER Q WITH TILDE -->
        </local:expansions>
    </xsl:variable>
    ...
    <!-- limit automatic tagging to sections in Spanish only, and cut out those g that are already tagged as abbreviations -->
    <xsl:template match="tei:text[@xml:lang eq 'es']//tei:g[not(ancestor::*[@xml:lang and @xml:lang ne 'es']) 
    ...
    <xsl:variable name="logG" as="xs:integer" select="count(//tei:text[@xml:lang eq 'es']//tei:g[(not(ancestor::*[@xml:lang and @xml:lang ne 'es']) and not(ancestor::tei:choice) and not(ancestor::tei:note))
                                                                                 and
