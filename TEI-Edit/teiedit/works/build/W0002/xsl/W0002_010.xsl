...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <xsl:message select="concat('Removed ', string($removedNewlines), ' text nodes containing only newlines.')"></xsl:message>
        <xsl:if test="not(($hyphenatingPb + $hyphenatingCb + $hyphenatingLb) eq $removedHyphens)">
        ...
        <xsl:if test="not($breakingLb eq ($hyphenatingPb + $hyphenatingCb))">
            <xsl:message terminate="no">Error: the number of lb elements marked (only) as break="no" does not align with the total number of 
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2018-08-02'"/>
    <xsl:param name="editingDesc" as="xs:string" select="'Annotated (hyphen-based) breaks.'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0002_change_0014'"/>
    ...
    <xsl:function name="local:isHyphenation" as="xs:boolean">
        ...
        <xsl:if test="not($elem[self::tei:pb or self::tei:cb or self::tei:lb])">
            <xsl:message terminate="yes" select="'Error: invalid element used for local:isHyphenation().'"/>
        </xsl:if>
        <xsl:choose>
            <xsl:when test="not($elem[self::tei:pb or self::tei:cb or self::tei:lb])">
            ...
            <xsl:when test="$elem[(preceding-sibling::node()[1]/self::text() and matches(preceding-sibling::node()[1], '-\n$'))
                                  or (preceding-sibling::*[1]/self::tei:hi and matches(preceding-sibling::*[1]/text()[last()], '-$'))]">
    ...
    <xsl:function name="local:removeHyphen">
        <xsl:param name="textNode" as="text()"/>
            <xsl:choose>
                <!-- hyphens at the end of a line wrapped in hi (if there are further such 'wrap' elements, they can be included here): -->
                ...
                <!-- all other text nodes ending with hyphen + \n -->
                <xsl:when test="matches($textNode, '-\n$') 
    ...
    <xsl:function name="local:isInterferingWhitespace" as="xs:boolean">
        ...
        <xsl:choose>
            <xsl:when test="$textNode[(following-sibling::node()[1][(self::tei:pb or self::tei:cb or self::tei:lb)])
                            and preceding-sibling::*[1]/self::tei:hi and ends-with(preceding-sibling::*[1]/text()[last()], '-')]">
                <xsl:value-of select="true()"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="false()"/>
    ...
    <xsl:variable name="removedHyphens" select="count(//text()[. ne local:removeHyphen(.)])"/>
    <xsl:variable name="removedNewlines" select="count(//text()[matches(., '\n') and (normalize-space(.) eq '') and local:isInterferingWhitespace(.)])"/>
