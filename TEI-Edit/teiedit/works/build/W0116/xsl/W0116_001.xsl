...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:local="http://salamanca.adwmainz.de" xmlns="http://www.tei-c.org/ns/1.0" exclude-result-prefixes="xs" xmlns:t="http://www.tei-c.org/ns/tite/1.0" version="2.0">
    ...
    <xsl:template match="tei:body//tei:div1[@type ne 'title']" mode="round-1">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="nDiv" select="count(preceding::tei:div1[ancestor::tei:body and @type ne 'title']) + 1"/>
    ...
    <xsl:template match="tei:body//tei:div1//tei:div2" mode="round-2">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="partId" select="ancestor-or-self::tei:div1/@xml:id"/>
            <xsl:attribute name="xml:id">
                <xsl:choose>
                    <xsl:when test="contains(@n, '[')">
                        <xsl:variable name="Numb" select="replace(@n, '(\S)(\d{2})(\S)', '$2')"/>
                        <xsl:value-of select="concat('chap', $Numb, '_', $partId)"/>
                    </xsl:when>

                    <xsl:otherwise>
                        <xsl:value-of select="concat('chap', ./@n, '_', $partId)"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:attribute>
...
<xsl:template match="tei:milestone" mode="round-2">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
<xsl:choose>
<xsl:when test="contains(@n, '.')">
