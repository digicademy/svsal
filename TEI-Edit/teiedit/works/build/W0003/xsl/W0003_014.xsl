...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2019-07-01'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0003_change_024'"/>
    ...
    <xsl:template match="/">
        ...
        <xsl:if test="count($choiceDuplicatesMain) gt 0">
            <xsl:for-each select="$choiceDuplicatesMain">
                <xsl:message select="concat('WARN: found adjacent choice/', ./local-name(), ' with identically resolved text - resolve this manually! (in line ', ./preceding::tei:lb[@xml:id][1]/@xml:id, ' )')"/>
            </xsl:for-each>
            <xsl:message terminate="yes" select="concat('WARN: found ', count($choiceDuplicatesMain), ' cases of adjacent choice with identically resolved text in the main area.')"/>
            <!-- setting to 'no' here since there is one case that is fine -->
        </xsl:if>
