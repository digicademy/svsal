...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0003_change_034'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Correction of notes: adding @anchored and @type to ref-elements.'"/>
    
    
    
    <xsl:template match="tei:teiHeader/tei:revisionDesc">
        <xsl:copy>
            <xsl:attribute name="status" select="'h_revised'"></xsl:attribute>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
    ...
    <xsl:template match="tei:teiHeader/tei:revisionDesc/tei:listChange">
        <xsl:copy>
            ...
            <xsl:element name="change">
                ...
                <xsl:attribute name="status" select="'h_revised'"/>
