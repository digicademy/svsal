...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2025-05-14'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0003_change_040'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Corrected pb/@n.'"/>
    ...
    <xsl:template match="tei:pb[not(@sameAs) and ancestor::tei:body]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="n" select="count(preceding::tei:pb[not(@sameAs) and ancestor::tei:body]) + 1"/>
