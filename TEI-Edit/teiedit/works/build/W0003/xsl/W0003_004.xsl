...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2019-02-20'"/>
    <xsl:param name="editingDesc" as="xs:string" select="'Tagged special characters.'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0003_change_0005'"/>
                ...
                <xsl:analyze-string select="." regex="{$specialCharsRegex}"> 
                    ...
                    <xsl:non-matching-substring>
                        ...
                        <xsl:if test="matches(., '[&#x0100;-&#x10ffff;]')">
                            <xsl:message terminate="no" select="concat('Error: found undeclared special character in text node: ', .)"/>
                        </xsl:if>
                        <xsl:value-of select="."/>
                    </xsl:non-matching-substring>
                </xsl:analyze-string>
