...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2021-10-25'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0037_Vol04_change_007'"></xsl:param>
...
<!-- 1) Final ũ|ū - um, legũ, appellatũ"-->
    ...
    <!--  Repeating final-um, so abbr. right after the same case can be also tagged. -->
    <xsl:variable name="final-um2">
        <xsl:apply-templates select="$final-um" mode="final-um2"/>
    </xsl:variable>
        
    <!-- identity transforms -->
    <xsl:template match="@*|node()" mode="final-um2">
        <xsl:copy>
            <xsl:apply-templates select="@*|node()" mode="final-um2"/>
        </xsl:copy>
    </xsl:template>
    
    <xsl:template match="text()[not(ancestor::tei:abbr or ancestor::*[@xml:lang = ('es','grc','gr','he','fr','pt','it')])]" mode="final-um2">
        <xsl:analyze-string select="." regex="{'(\s)([æœęaA-zZſç]+)(ũ|ũ|ū|ū)([, \?!\(\)\*\+✝]+)'}">
            <xsl:matching-substring>
                <xsl:value-of select="regex-group(1)"/>
                <xsl:element name="abbr">
...
<!-- 2) Final ā|ã - am, primā, verā"-->
    ...
    <xsl:variable name="final-am">
        <xsl:apply-templates select="$final-um2" mode="final-am"/>
    </xsl:variable>
    ...
    <!--  Repeating final-am, so abbr. right after the same case can be also tagged. -->
    <xsl:variable name="final-am2">
        <xsl:apply-templates select="$final-am" mode="final-am2"/>
    </xsl:variable>
    
    <!-- identity transforms -->
    <xsl:template match="@*|node()" mode="final-am2">
        <xsl:copy>
            <xsl:apply-templates select="@*|node()" mode="final-am2"/>
        </xsl:copy>
    </xsl:template>
    
    <xsl:template match="text()[not(ancestor::tei:abbr or ancestor::*[@xml:lang = ('es','grc','gr','he','fr','pt','it')])]" mode="final-am2">
        <xsl:analyze-string select="." regex="{'(\s)([æœęaA-zZſç]+)(ã|ã|ā|ā)([, \?!\(\)\*\+✝]+)'}">
            <xsl:matching-substring>
                <xsl:value-of select="regex-group(1)"/>
                <xsl:element name="abbr">
...
<!--3)  ā + final di|dum|t|ti|tibus|tis|tur, an, mode="antur"-->
    ...
    <xsl:variable name="antur">
        <xsl:apply-templates select="$final-am2" mode="antur"/>
    </xsl:variable>
