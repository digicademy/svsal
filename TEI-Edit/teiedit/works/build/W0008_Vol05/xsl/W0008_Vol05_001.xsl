...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2023-01-31'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0008_Vol05_change_009'"></xsl:param>
...
<!-- div/@n -->
<xsl:template match="tei:body//tei:div2[not(@xml:id)]">
         <xsl:copy>
<xsl:copy-of select="@*"/>
 ...
 <!-- Added @target(s) to ref(s) in index.   -->
    ...
    <xsl:template match="tei:front//tei:ref[not(@target)]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="ref" select="count(preceding::*//tei:ref[not(@target) and ancestor::tei:front])+1"/>
