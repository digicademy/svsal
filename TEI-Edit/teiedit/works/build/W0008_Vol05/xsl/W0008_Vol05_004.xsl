...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="workId" as="xs:string" select="'W0008_Vol05'"/>
    <xsl:param name="textId" as="xs:string" select="'Vol05'"/> <!-- "completeWork" (for the whole work) or "Vol_xx" (for volume xx) -->
    ...
    <xsl:param name="volumeNumber" as="xs:integer" select="05"/> <!-- number of the volume in a multi-volume work (must be "0" for single-volume works)  -->
...
<!--    <xsl:param name="analyzeTextEnabled" as="xs:boolean" select="true()"/> <!-\- this doens't need to be modified -\->-->
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2023-02-02'"/>
    <xsl:param name="editingDesc" as="xs:string" select="'Transformation from TEI-Tite to TEI-All.'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0008_Vol05_change_012'"/>
    ...
    <xsl:include href="W0008_Vol05_004b.xsl"/>
    ...
    <teiHeader>
        <fileDesc>
            ...
            <titleStmt>
                ...
                <title type="main" level="m">R. P. Lodovici Molinae, Primarii Qvondam In Eborensi Academia Sac. Theologiæ 
                    Professoris, E Societate Iesv, De Ivstitia Et Ivre Tomvs Qvartvs De Justitia 
                    Commutatiua circa bona corporis, perfonarumq[ue], nobis coniunctarum</title>
        ...
        <revisionDesc status="a_raw">
            ...
            <change who="#PS #auto" when="2023-02-02" status="a_raw" xml:lang="en" xml:id="W0008_Vol05_change-011">Added (la) abbreviations depending on word structure with regex.</change>
            <change who="#PS #auto" when="2023-01-31" status="a_raw" xml:lang="en" xml:id="W0008_Vol05_change-010">Added cross references in summaries.</change>
            <change who="#PS #auto" when="2023-01-31" status="a_raw" xml:lang="en" xml:id="W0008_Vol05_change-009">Added pagination, added @n and @xml:id to div2, added toc.</change>
            <change who="#PS" when="2022-11-02" status="a_raw" xml:lang="en" xml:id="W0008_Vol05_change-008">Structural annotation.</change>
