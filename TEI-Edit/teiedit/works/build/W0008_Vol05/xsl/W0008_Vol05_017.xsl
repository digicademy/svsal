...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0008_Vol05_change_024'"/>
    ...
    <xsl:template match="tei:body//tei:cb[not(@type eq 'end')]" mode="postcorr-fixes">
        <xsl:copy>
        <xsl:copy-of select="@*"/>
                <xsl:variable name="n" select="count(./preceding::*//self::tei:cb[not(@type eq 'end')][ancestor::tei:body])+1693"/>           
