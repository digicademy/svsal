...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2026-04-02'"/>
    <xsl:param name="editingDesc" as="xs:string" select="'Tagged special characters characters after manual corrections.'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0008_Vol05_change_027'"/>
                ...
                <xsl:analyze-string select="." regex="{$specialCharsRegex}"> 
                    ...
                    <xsl:non-matching-substring>
                        ...
                        <xsl:if test="matches(., '[&#x0100;-&#x10ffff;]')">
                            <xsl:message terminate="yes" select="concat('Error: found undeclared special character in text node: ', $thisText/preceding::tei:lb[1]/@xml:id)"/><!--, 'if not string is found, go ahead and add xml:id(s) and then replace the varible $thisText:| after element with xml:id: ', $thisText/preceding::*[@xml:id]/@xml:id)-->
                        </xsl:if>
                        <xsl:value-of select="."/>
                    </xsl:non-matching-substring>
                </xsl:analyze-string>
