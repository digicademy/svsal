...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0034_change_09'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Created new @xml:id and line numbering.'"/>
        ...
        <xsl:copy>
            ...
            <xsl:if test="@sameAs">
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:message terminate="yes" select="concat('Error: key element variable for lb has no value, or an unacceptable value: ', substring-after(@sameAs, '#'))"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:if>
        </xsl:copy>
