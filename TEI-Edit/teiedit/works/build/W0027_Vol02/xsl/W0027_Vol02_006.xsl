...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2026-07-28'"/>
    <xsl:param name="editingDesc" as="xs:string" select="'Tagged special characters.'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0027_Vol02_change_008'"/>
                ...
                <xsl:analyze-string select="." regex="{$specialCharsRegex}"> 
                    ...
                    <xsl:non-matching-substring>
                        ...
                        <xsl:if test="matches(., '[&#x0100;-&#x10ffff;]')">
                            <xsl:message terminate="no" select="concat('Error: found undeclared special character in text node: ', $thisText)"/><!--, 'if not string is found, go ahead and add xml:id(s) and then replace the varible $thisText:| after element with xml:id: ', $thisText/preceding::*[@xml:id]/@xml:id)-->
                        </xsl:if>
                        <xsl:value-of select="."/>
                    </xsl:non-matching-substring>
                </xsl:analyze-string>
