...
Replace line 113 with the respective <change> e.g.: 
...
text = open("D:/SVN/trunk/teiedit/works/build/W0027_Vol02/xml/W0027_Vol02_011.xml", encoding='utf-8').read()
...
# 3 Latin patterns in regular expressions (key) saved as variables e.g. "final_um"
...
# 4 Dictionary with regular expressions:
...
cases = {
    ...
    '<listChange ordered="true">\n                ' : '<listChange ordered="true">\n                <change who="#CR #auto" when="2026-07-30" status="a_raw" xml:id="W0027_Vol02_change_013" xml:lang="en">Automatically expanded abbreviations with lb/@break using python.</change>\n                '
}
...
# 5 add xml:ids to //tei:expan[@resp = '#CRPY #auto']//tei:lb[@sameAs] using lxml ---
...
def _make_suffix(counter: int) -> str:
    return _base36(counter).rjust(4, "0")  # 0000..zzzz

parser = etree.XMLParser(remove_blank_text=False, recover=True, encoding="utf-8")
doc = etree.parse(BytesIO(text_output.encode("utf-8")), parser)
root = doc.getroot()

lbs = root.xpath(".//tei:expan[@resp = '#CRPY #auto']//tei:lb[@sameAs]", namespaces=ns)

existing_ids = set(root.xpath(".//@xml:id", namespaces=ns))
new_ids = set()
...
for lb in lbs:
    ...
    if not same_as or not same_as.startswith("#"):
        continue

    base = same_as[1:]  # without '#'
    prefix = base[:-4] if len(base) >= 4 else (base + "-")

    while True:
        suffix = _make_suffix(counter)
        counter += 1
        candidate = f"{prefix}{suffix}"
        if candidate not in existing_ids and candidate not in new_ids:
            break

    lb.set(f"{{{XML_NS}}}id", candidate)
    new_ids.add(candidate)
    assigned += 1

text_output = etree.tostring(doc, encoding="UTF-8", xml_declaration=True).decode("UTF-8")

# Check if the xml:ids were tagged.
print("Assigned new xml:ids to lb elements:", assigned)

# --- END TEI xml:ids for //expan[@resp eq '#CRPY #auto']//lb[@sameAs] BLOCK ---

# 6. Save output: a xml copy with the new results.

"""
W0006_Vol02_013_prueba =open('D:/SVN/trunk/teiedit/works/build/W0006_Vol02/xml/W0006_Vol02_013.xml','w', encoding='utf-8')
W0006_Vol02_013_prueba.write(text_output)
W0006_Vol02_013_prueba.close()"""


W0027_Vol02 =open('D:/SVN/trunk/teiedit/works/build/W0027_Vol02/xml/W0027_Vol02_012.xml', 'w', encoding='utf-8')
W0027_Vol02.write(text_output)
W0027_Vol02.close()

expan_added = re.findall(r'#CRPY', text_output)


print("1 Check the <revisionDesc//listChange//change. Was this last step with python added?. If not, check line 111.")
