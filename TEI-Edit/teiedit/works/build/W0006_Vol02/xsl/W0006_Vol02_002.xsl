...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2020-08-07'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0006_Vol02_change-011'"></xsl:param>
    ...
    <!--Adding @type, @n and @xml:id to div2(s).-->
     <xsl:template match="tei:body//tei:div2[not(@type)]">
         <xsl:copy>
         <xsl:copy-of select="@*"/>
             <xsl:variable name="nchap" select="count(preceding-sibling::*//self::tei:div2[not(@type) and ancestor::tei:body])+1"/>
    ...
    <!--Adding @target to ref(s) in content lists-->
    ...
    <xsl:template match="tei:list[@xml:id eq 'listb3']//tei:ref">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="ref" select="count(preceding::*//self::tei:ref[ancestor::tei:list[@xml:id eq 'listb3']])+1"/> 
            <xsl:attribute name="target" select="concat('#cap',$ref,'b3')"/>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>

    <xsl:template match="tei:list[@xml:id eq 'listb4']//tei:ref">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="ref" select="count(preceding::*//self::tei:ref[ancestor::tei:list[@xml:id eq 'listb4']])+1"/> 
            <xsl:attribute name="target" select="concat('#cap',$ref,'b4')"/>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
