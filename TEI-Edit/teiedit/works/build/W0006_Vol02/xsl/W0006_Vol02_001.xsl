...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2020-08-03'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0006_Vol02_change-010'"/>
    <xsl:param name="editingDesc" as="xs:string" select="'Added missing pagination.'"/>
    ...
    <!--Adding pagination in book 4.-->
    <xsl:template match="tei:div1[@xml:id eq 'b4']//tei:pb[not(@sameAs)]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:attribute name="n">
                <xsl:variable name="currentPage" as="xs:string">
                    <xsl:number value="count(preceding::tei:pb[not(ancestor::tei:note and @sameAs)]) - 487"/>
                </xsl:variable>
    ...
    <!--Adding pagination in back matter.-->
    <xsl:template match="tei:back//tei:pb[not(@sameAs)]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:attribute name="n">
                <xsl:variable name="n" as="xs:string">
                    <xsl:number value="count(preceding::tei:pb[ancestor::tei:back and not(ancestor::tei:note and @sameAs)]) + 128"/>
                </xsl:variable>
