...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2019-05-13'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0017_change_005'"></xsl:param>
    ...
    <xsl:template match="text()[matches(.,'[-=]\n?$') and following-sibling::node()[1][(self::tei:pb or self::tei:cb or self::tei:lb or self::tei:note )]]" mode="breakLB">
        <xsl:choose>
    ...
    <xsl:variable name="Newline">
        <xsl:apply-templates select="$breakPB_CB" mode="Newline"/>
    </xsl:variable>
    
    <!-- identity transform -->
    <xsl:template match="@*|node()" mode="Newline">
        <xsl:copy>
            <xsl:apply-templates select="@*|node()" mode="Newline"/>
        </xsl:copy>
    </xsl:template>
    
    <xsl:template match="text()[normalize-space() eq '' and following-sibling::node()[1]/(self::tei:pb[@break='no'] or self::tei:cb[@break='no'] or self::tei:cb[@break='no'])]" mode="Newline"/>
