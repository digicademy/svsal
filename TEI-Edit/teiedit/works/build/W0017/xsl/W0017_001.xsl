...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2018-08-29'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0017_change_02'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'div2|div4 @type Annotation'"/>
    ...
    <xsl:template match="//tei:text//tei:div2[not(@type eq 'preface')]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:attribute name="type" select="'title'"/>
            <!--<xsl:message>Info: created @type and @n</xsl:message>-->
            <xsl:attribute name="n">
                <xsl:value-of select="count(preceding-sibling::tei:div2)"/>
            </xsl:attribute>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
    
    <xsl:template match="tei:div3[@type='chapter']">
        <xsl:choose>
            <xsl:when test="not(ancestor::tei:div2[19])">
                <xsl:copy>
                    <xsl:variable name="n1" select="count(preceding-sibling::tei:div3)+1"/>
                    ...
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <xsl:attribute name="n" select="$n1"/>
                        </xsl:otherwise>
                    </xsl:choose>
                    <xsl:apply-templates/>
                </xsl:copy>
            </xsl:when>
            <xsl:when test="ancestor::tei:div2[19]">
                <xsl:copy>
                    <xsl:variable name="n2" select="count(preceding-sibling::tei:div3)"/>
                    <xsl:copy-of select="@* except @n"/>
                    <xsl:choose>
                        <xsl:when test="contains(@n,'[')">
                            <xsl:copy-of select="@n"/>
                        ...
                        <xsl:otherwise>
                            <xsl:attribute name="n" select="$n2"/>
                        </xsl:otherwise>
                    </xsl:choose>
                    <xsl:apply-templates/>
                </xsl:copy>
    ...
    <xsl:template match="//tei:text//tei:div4">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:attribute name="type" select="'section'"/>
            <!--<xsl:message>Info: created @type</xsl:message>-->
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
    <xsl:template match="tei:div1[@type='index']//tei:list">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
