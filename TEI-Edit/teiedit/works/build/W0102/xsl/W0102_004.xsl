...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0102_change_007'"/>
    ...
    <xsl:template match="tei:lb[@type eq 'nb'][not(preceding-sibling::*[1]/self::tei:note)]" mode="breakLB">
        <xsl:copy>
    ...
    <xsl:template match="tei:lb[@type eq 'nb'][preceding-sibling::*[1]/self::tei:note]" mode="breakLB">
        <xsl:copy>
            <xsl:attribute name="break" select="'no'"/>
            <xsl:attribute name="rendition" select="'#hyphen'"/>
            <xsl:apply-templates mode="breakLB"/>
