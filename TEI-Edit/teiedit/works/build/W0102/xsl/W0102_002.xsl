...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2025-12-09'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0102_change_00X'"/>
...
<xsl:otherwise>
    ...
    <xsl:variable name="non">
        <xsl:apply-templates select="$final-am" mode="non"/>
    </xsl:variable>
    
    <!-- identity transforms -->
    <xsl:template match="@*|node()" mode="non">
        <xsl:copy>
            <xsl:apply-templates select="@*|node()" mode="non"/>
        </xsl:copy>
    </xsl:template>
    
    <xsl:template match="text()[not(ancestor::tei:abbr or ancestor::*[@xml:lang = ('es','grc','gr','he','fr','pt','it')])]" mode="non">
        <xsl:analyze-string select="." regex="{'(\s)(n)(õ|õ|ō|ō)([, :;\.\?!\)\n]+)'}">
            <xsl:matching-substring>
                <xsl:value-of select="regex-group(1)"/>
                <xsl:element name="abbr">
                    <xsl:attribute name="rend" select="'choice'"/>
...
<!--3)  ā + final di|dum|t|ti|tibus|tis|tur, an, mode="antur"-->
    ...
    <xsl:variable name="antur">
        <xsl:apply-templates select="$non" mode="antur"/>
    </xsl:variable>
...
<!--Added 3 words in foreign/abbr in spanish.-->
<!--cambiar-->
    <xsl:variable name="cambiar">
        <xsl:apply-templates select="$sct" mode="cambiar"/>
    </xsl:variable>
    
    <!-- identity transforms -->
    <xsl:template match="@*|node()" mode="cambiar">
        <xsl:copy>
            <xsl:apply-templates select="@*|node()" mode="cambiar"/>
        </xsl:copy>
    </xsl:template>
    
    <xsl:template match="tei:foreign//text()[not(ancestor::tei:abbr or ancestor::*[@xml:lang = ('la','grc','gr','he','fr','pt','it')])]" mode="cambiar">
        <xsl:analyze-string select="." regex="{'(\s)(c)(ã|ã|ā)(biar)([ \.,:;\(\)])'}">
            <xsl:matching-substring>
                <xsl:value-of select="regex-group(1)"/>
                <xsl:element name="abbr">
                    <xsl:attribute name="rend" select="'choice'"/>
                    ...
                    <xsl:element name="abbr">
                        <xsl:attribute name="rend" select="'expan'"/>
                        <xsl:attribute name="resp" select="'#CR #auto'"/>
                        <xsl:value-of select="concat(regex-group(2),'am',regex-group(4))"/>
                    </xsl:element>
                </xsl:element>
                <xsl:value-of select="regex-group(5)"/>
            </xsl:matching-substring>
...
<!--ſemejantes-->
    <xsl:variable name="ſemejantes">
        <xsl:apply-templates select="$cambiar" mode="ſemejantes"/>
    </xsl:variable>
    
    <!-- identity transforms -->
    <xsl:template match="@*|node()" mode="ſemejantes">
        <xsl:copy>
            <xsl:apply-templates select="@*|node()" mode="ſemejantes"/>
        </xsl:copy>
    </xsl:template>
    
    <xsl:template match="tei:foreign//text()[not(ancestor::tei:abbr or ancestor::*[@xml:lang = ('la','grc','gr','he','fr','pt','it')])]" mode="ſemejantes">
        <xsl:analyze-string select="." regex="{'(\s)(ſemej)(ã|ã|ā)(tes)([ \.,:;\(\)])'}">
            <xsl:matching-substring>
                <xsl:value-of select="regex-group(1)"/>
                <xsl:element name="abbr">
                    <xsl:attribute name="rend" select="'choice'"/>
...
<!--algun-->
    <xsl:variable name="final-un">
        <xsl:apply-templates select="$ſemejantes" mode="final-un"/>
    </xsl:variable>
...
-->
    ...
    <!--lb rendition hyphen-->
    <xsl:variable name="Put-LBHyphen-back">
        <xsl:apply-templates select="$final-un" mode="Put-LBHyphen-back"/>
    </xsl:variable>
