...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0008_Vol07_change_009'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Added pagination, added @n and @xml:id to div2, added toc.'"/>
...
<xsl:template match="tei:back//tei:pb[not(@sameAs)]">
         <xsl:copy>
         <xsl:copy-of select="@*"/>
             <xsl:variable name="pbn" select="count(preceding::*//tei:pb[not(@sameAs) and ancestor::tei:back])+265"/>
         <xsl:attribute name="n" select="concat('[',$pbn,']')"/>
             <xsl:apply-templates/>
         </xsl:copy>
     </xsl:template>
    ...
    <!--Added cb/@n in body-->
    <xsl:template match="tei:body//tei:cb[not(@sameAs)]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="n" select="count(./preceding::*//self::tei:cb[not(@sameAs)][ancestor::tei:body])+1"/>           
            <xsl:choose>
...
<!-- div/@n -->
<xsl:template match="tei:body//tei:div2">
         <xsl:copy>
<xsl:copy-of select="@*"/>
             <xsl:variable name="div2" select="count(preceding::tei:div2)+1"/>
         <xsl:attribute name="n" select="$div2"/>
         <xsl:attribute name="xml:id" select="concat('disp',$div2)"/>
             <xsl:apply-templates/>
         </xsl:copy>
    </xsl:template>
 ...
 <!-- Added @target(s) to ref(s) in index.   -->
    ...
    <xsl:template match="tei:front//tei:ref">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="ref" select="count(preceding::*//tei:ref[ancestor::tei:front])+1"/>
