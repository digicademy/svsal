...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0001_Vol04_change_013'"/>
    ...
    <xsl:template match="tei:pb[not(@break)]" mode="round2">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:if test="./following-sibling::*[1]/self::tei:cb and ./following-sibling::*[2]/self::tei:lb[@break eq 'no']">
                <xsl:attribute name="break" select="'no'"/>
                <xsl:attribute name="rendition" select="'#hyphen'"/>
            </xsl:if>
        </xsl:copy>
