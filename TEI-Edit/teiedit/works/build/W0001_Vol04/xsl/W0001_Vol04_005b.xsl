...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="tei:lb">
        <xsl:copy>
            ...
            <xsl:choose>
                ...
                <xsl:when test="@type"><xsl:message terminate="yes" select="'ERROR: unknown lb/@type'"/></xsl:when>
                <xsl:otherwise/>
            </xsl:choose>
        </xsl:copy>
    </xsl:template>
    ...
    <xsl:template match="*[not(self::tei:hi or self::tei:p or self::tei:milestone)][@rend or @rendition]">
    ...
    <xsl:template match="comment()" priority="2">
        <xsl:choose>
            <xsl:when test="matches(normalize-space(.), '^tao:A[-\+][1-5](beg|end)$')"/> <!-- delete font size information -->
            ...
            <xsl:when test="matches(normalize-space(.), '^tao:A[-\+][1-5][_](beg|end)$')"/>
            <xsl:otherwise>
