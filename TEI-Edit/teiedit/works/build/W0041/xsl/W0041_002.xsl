...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0041_change_005'"></xsl:param>
...
<!-- 18) q + ´ + ; ==> que, leuisq́;, mode="qac"-->
    ...
    <xsl:template match="text()[not(ancestor::tei:abbr or ancestor::*[@xml:lang = ('es','grc','gr','he','fr','pt','it')])]" mode="qac">
        <xsl:analyze-string select="." regex="{'(\s)([æœęaA-zZñſç]+)(q́;)([, \?!\(\)\*\+✝]+)'}">
            <xsl:matching-substring>
                <xsl:value-of select="regex-group(1)"/>
                <xsl:element name="abbr">
                    <xsl:attribute name="rend" select="'choice'"/>
...
<!-- 19) q3 + ´ (chare8bf0301) Exemplum́  mode="q3accent"-->
    ...
    <xsl:template match="text()[not(ancestor::tei:abbr or ancestor::*[@xml:lang = ('es','grc','gr','he','fr','pt','it')])]" mode="q3accent">
        <xsl:analyze-string select="." regex="{'(\s)([æœęaA-zZñſç]+)(́)([, \?!\(\)\*\+✝]+)'}">
            <xsl:matching-substring>
                <xsl:value-of select="regex-group(1)"/>
                <xsl:element name="abbr">
                    <xsl:attribute name="rend" select="'choice'"/>
...
<!-- 20) q3 (chare8bf), mode="q3"-->
    ...
    <xsl:template match="text()[not(ancestor::tei:abbr or ancestor::*[@xml:lang = ('es','grc','gr','he','fr','pt','it')])]" mode="q3">
        <xsl:analyze-string select="." regex="{'(\s)([æœęaA-zZñſç]+)()([, \?!\(\)\*\+✝]+)'}">
            <xsl:matching-substring>
                <xsl:value-of select="regex-group(1)"/>
                <xsl:element name="abbr">
                    <xsl:attribute name="rend" select="'choice'"/>
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="Abbr" as="node()*" select="$q3//tei:abbr[@rend eq 'abbr' and following-sibling::node()/self::tei:abbr[@rend eq 'expan' and matches(.,'[̃ ãāēẽõōũūꝓđ́]+')]]"/>
        <xsl:variable name="WrongExpansions" as="node()*" select="$q3//tei:abbr[@rend eq 'choice']//tei:abbr[@rend eq 'expan' and matches(.,'[̃ ãāēẽõōũūꝓđ́]+')]"/>
        <xsl:choose>
            ...
            <!--Abbr with no special character, check this out.-->
            <xsl:when test="$q3//tei:abbr[@rend eq 'abbr' and not(matches(.,'[ẽãāāēẽõōũūꝓꝰ́q́]+'))]">
            ...
            <xsl:otherwise>
                ...
                <xsl:variable name="Expansions" as="xs:integer" select="count($q3//tei:abbr[@rend eq 'choice']//tei:abbr[@rend eq 'expan'])"/>
