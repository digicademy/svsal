...
<!--Content and Index in nested lists-->

    <!--<xsl:param name="editors" as="xs:string" select="'#CR #DG #auto'"/>
    <xsl:param name="editingDate" as="xs:string" select="'2019-xx-xx'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0001_Vol06_change_XXX'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Added @type to lists in contents and nested lists in Index'"/>
...
<!-- Added @type in list: Table of contents W0001-F-0010 - W0001-F-0014   -->
    <xsl:template match="tei:list[ancestor::tei:list[@type='contents']]">
        <xsl:copy>
...
<!--Added @type in list: Index W0001-F-0532-->
    <xsl:template match="tei:list[ancestor::tei:div1[@xml:id eq 'SACRÆ_SCRIPTVRÆ']]">
        <xsl:copy>
            <xsl:attribute name="type" select="'index'"/>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
<!-- Nested lists in index W0001-F-0537-->
    <xsl:template match="tei:list[@type='index'][ancestor::tei:div1[@xml:id eq 'INDEX_RERVM']]//tei:item[not(descendant::tei:list)]">
        <xsl:copy>
            <xsl:choose>
                <xsl:when test="ancestor::tei:list[@type='index']">
    ...
    <xsl:template match="tei:list[@type='index'][ancestor::tei:div1[@xml:id eq 'INDEX_RERVM']]//tei:item//tei:p[1][not(ancestor::tei:argument)]">
    ...
    <xsl:template match="tei:list[@type= 'index'][ancestor::tei:div1[@xml:id eq 'INDEX_RERVM']]//tei:item//tei:p[preceding-sibling::tei:p]"> 
