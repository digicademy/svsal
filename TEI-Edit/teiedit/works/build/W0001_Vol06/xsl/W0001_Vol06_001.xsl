...
<!--Added missing pagination (pb/@n).   -->
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2019-08-08'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0001_Vol06_change_xxx'"/>
    ...
    <xsl:template match="tei:back//tei:pb[not(@sameAs)]">
        <xsl:variable name="n" as="xs:integer" select="count(./preceding::tei:pb[not(@sameAs)][ancestor::tei:back])+517"/>
        <xsl:copy>
            <xsl:copy-of select="@* except @n"/>
            <xsl:choose>
                <xsl:when test="contains(@n,'[')">
