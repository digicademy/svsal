...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2019-08-16'"/>
    ...
    <xsl:template match="tei:div2[not(@type)]">
        <xsl:choose>
            <xsl:when test="ancestor::tei:div1[@xml:id = ('p1','p2','p4','p5','p6')]">
                <xsl:copy>
                    <xsl:copy-of select="@*"/>
            ...
            <xsl:when test="ancestor::tei:div1[@xml:id eq 'p3']">
                <xsl:copy>
                    <xsl:copy-of select="@*"/>
    ...
    <xsl:template match="tei:div3[not(@type)]|tei:div4[not(@type)]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:attribute name="type" select="'section'"/>
            <xsl:apply-templates/>
