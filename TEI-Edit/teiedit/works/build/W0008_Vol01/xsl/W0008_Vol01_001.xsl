...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2021-03-11'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0008_Vol01_change-008'"></xsl:param>
...
<xsl:template match="tei:back//tei:pb[not(@sameAs)]">
         <xsl:copy>
         <xsl:copy-of select="@*"/>
             <xsl:variable name="pbn" select="count(preceding::*//tei:pb[not(@sameAs) and ancestor::tei:back])+773"/>
         <xsl:attribute name="n" select="concat('[',$pbn,']')"/>
             <xsl:apply-templates/>
         </xsl:copy>
     </xsl:template>
