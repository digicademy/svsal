...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2022-02-01'"/> <!--Updated: 22.05.24 CR-->
    <xsl:param name="changeId" as="xs:string" select="'W0008_Vol01_change-009'"/>
...
<!-- 
    ...
    * Every template has a case and a mode:
    ...
    Updated 27.04.23
    Names were removed and added to the abbreviation list: /SVN/trunk/teiedit/works/resources/config/abbr-la.xml
    ...
    They can produce false negatives as:
    
    Alexam instead of Alexan.
...
<!-- 2) Final ā|ã - am, primā, verā"-->
    ...
    <xsl:variable name="final-am">
        <xsl:apply-templates select="$Ioan" mode="final-am"/>
    </xsl:variable>
...
<!--16) ⁊ (char204a) ==> et. , mode="only-et"-->
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="WrongExpansions" as="node()*" select="$only-et//tei:abbr[@rend eq 'expan' and matches(.,'[̃ ãāēẽõōũūꝓđ́]+')]"/>
        <xsl:choose>
