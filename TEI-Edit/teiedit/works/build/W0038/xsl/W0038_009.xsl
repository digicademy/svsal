...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2025-06-16'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0038_change_010'"/>
    ...
    <xsl:template match="tei:text//tei:cb">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
<xsl:message terminate="no" select="concat('CB: ', ./@n)"/>
...
<xsl:message terminate="no" select="concat('Milestone: ', ./@xml:id)"/>
        <xsl:copy>
            <xsl:copy-of select="@* except @xml:id"/>
            <xsl:attribute name="xml:id">
...
<xsl:message terminate="no" select="concat('Ref: ', ./@xml:id)"/>
        <xsl:copy>
            <xsl:copy-of select="@*"/>
...
<xsl:message terminate="no" select="concat('Anchor-Node: ', $anchorNode)"/>
<xsl:message terminate="no" select="concat('Pb-ID: ', $anchorNode/preceding::tei:pb[not(@sameAs)][1]/@xml:id)"/>
