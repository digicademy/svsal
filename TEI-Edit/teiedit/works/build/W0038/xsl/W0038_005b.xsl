...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="@*|node()">
        <!-- if there is any node not explicitely defined further below, terminate:  local-name(.)-->
        <xsl:message terminate="yes" select="concat('Error: encountered node not specified in the transformation: ', ./@xml:id)"/>
        <!--<xsl:copy>
...
<xsl:message terminate="no" select="concat('targetNode:', $targetNode)"/>
