    ...
    <xsl:template match="tei:note[@type='margin' and @rend='noRef']">
        ...
        <xsl:copy>
            ...
            <xsl:choose>
                ...
                <xsl:when test="text()[matches(.,'^\d+[\.:]?$') or matches(.,'§.? \d+[\.:]?$')]">
                    <xsl:message terminate="yes" select="concat('ERROR: note(s) with only digits should be converted into milestone(s). note:', ./@xml:id)"/>
                </xsl:when>
                <xsl:otherwise/>
            </xsl:choose>
            <xsl:copy-of select="@resp|@cert|@change|@xml:lang"/>
    ...
    <xsl:template match="tei:note[@type='margin' and @rend = ('asterisk', 'dagger')]">
        ...
        <xsl:copy>
            ...
            <xsl:choose>
                ...
                <xsl:when test="text()[matches(.,'^\d+[\.:]?$') or matches(.,'§.? \d+[\.:]?$')]">
                    <xsl:message terminate="yes" select="'ERROR: note(s) with only digits should be converted into milestone(s). note: '"/>
                </xsl:when>
                <xsl:otherwise/>
            </xsl:choose>
            <xsl:copy-of select="@resp|@cert|@change|@xml:lang"/>
