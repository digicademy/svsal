...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0098_change_010'"/>
    ...
    <xsl:variable name="corrLists" as="document-node()+" 
        select="for $path in tokenize($corrListPaths, ',') return if (doc-available($path)) then doc($path) else concat('corrListPaths ', $corrListPaths)"/>
