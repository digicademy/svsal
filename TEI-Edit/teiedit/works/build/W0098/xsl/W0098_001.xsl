...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:local="http://salamanca.adwmainz.de" xmlns="http://www.tei-c.org/ns/1.0" exclude-result-prefixes="xs" xmlns:t="http://www.tei-c.org/ns/tite/1.0" version="2.0">
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2025-07-03'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0098_change_003'"/>
...
<xsl:message terminate="no" select="concat('$nDiv: ', $nDiv)"/>
    ...
    <xsl:template match="tei:body//tei:div1//tei:div2[not(@type)]" mode="round-1">
        <xsl:copy>

            <xsl:copy-of select="@*"/>
            <xsl:attribute name="type" select="'chapter'"/>
            <xsl:variable name="nDiv" select="count(preceding-sibling::tei:div2[ancestor-or-self::tei:div1 and not(@type)]) + 1"/>
   ...
   <xsl:template match="tei:front//tei:div1/tei:div2" mode="round-2">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
        
<xsl:attribute name="type" select="'privileges'"/>
            <xsl:apply-templates mode="round-2"/>
        </xsl:copy>
    </xsl:template>

    <xsl:template match="tei:body//tei:div1//tei:div2[@n]" mode="round-2">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
...
<xsl:template  match="tei:body//tei:div1//tei:div2[@type eq 'conclusion']" mode="round-2">
 
        <xsl:copy>
            <xsl:copy-of select="@*"/>
  <xsl:variable name="bookId" select="ancestor-or-self::tei:div1/@xml:id"/>
<xsl:attribute name="xml:id" select="concat('conclusion_', $bookId)"/>
       <xsl:apply-templates mode="round-2"/>
        </xsl:copy>
</xsl:template>

 <xsl:template match="tei:front//tei:list/tei:item/tei:list" mode="round-2">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
 <xsl:variable name="nList" select="count(preceding::tei:list)+1"/>
    ...
    <!--Third round - div3 and ref/chapter and milestones-->
    ...
    <xsl:template match="//tei:div2//tei:div3" mode="round-3">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
<xsl:choose>
<xsl:when test=".[@type eq 'part']">
  <xsl:variable name="chapID" select="ancestor-or-self::tei:div2/@type"/>

            <xsl:attribute name="xml:id">
                <xsl:variable name="nPart" select="count(preceding-sibling::tei:div3[ancestor-or-self::tei:div2]) + 1"/>
                <xsl:value-of select="concat('part', $nPart, '_', $chapID, '_prooemium')"/>
...
<xsl:otherwise>
            ...
            <xsl:attribute name="xml:id">
                <xsl:variable name="nPart" select="count(preceding-sibling::tei:div3[ancestor-or-self::tei:div2]) + 1"/>
                <xsl:value-of select="concat('part', $nPart, '_', $chapID)"/>
...
<xsl:template match="tei:milestone" mode="round-3">
        <xsl:copy>
...
<!-- /tei:text/tei:front[1]/tei:div1[8]/tei:list[1]/tei:item[6]/tei:list[1]/tei:item[12]/tei:ref[1]-->
<xsl:template match="tei:front//tei:list[@xml:id]//tei:ref" mode="round-3">
 ...
 <xsl:attribute name="xml:id" select="concat('conlcusion_', $listId)"/>
<xsl:attribute name="target" select="concat('#conclusion_', substring-after($listId, '-'))"/>
</xsl:when>
<xsl:otherwise><xsl:attribute name="xml:id" select="concat('ref_chap', $nItem, '_', $listId)"/>
