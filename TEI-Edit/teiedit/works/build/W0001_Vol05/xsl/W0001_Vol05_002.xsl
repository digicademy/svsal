...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2019-07-17'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0001_Vol05_change_xxx'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Add @type to div2 and div3'"/>
    ...
    <xsl:template match="@*|node()">
     ...
     <xsl:template match="tei:div2[not(@type)][ancestor::tei:div1[@xml:id eq 'p1']]">
         <xsl:copy>
             <xsl:copy-of select="@*"/>
    ...
    <xsl:template match="tei:div2[not(@type)][ancestor::tei:div1[@xml:id eq 'a1']]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
    ...
    <xsl:template match="tei:div3[not(@type)][ancestor::tei:div1[@xml:id eq 'p1' or @xml:id eq 'a1']]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:attribute name="type" select="'section'"/>
            <xsl:apply-templates/>
    ...
    <xsl:template match="tei:div3[not(@type)][ancestor::tei:div1[@xml:id eq 'p2']]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
    ...
    <xsl:template match="tei:div4[not(@type)][ancestor::tei:div1[@xml:id eq 'p2']]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:attribute name="type" select="'section'"/>
            <xsl:apply-templates/>
