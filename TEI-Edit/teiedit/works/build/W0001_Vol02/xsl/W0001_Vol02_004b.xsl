...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="tei:hi">
        <xsl:choose>
            ...
            <xsl:when test="@rend = ('init', 'right', 'centered', 'spc', 'centered spaced', 'spaced italics')">
                <xsl:copy>
    ...
    <xsl:template match="*[not(self::tei:hi or self::tei:lb or self::tei:cb)][@rend or @rendition]">
    ...
    <xsl:template match="comment()" priority="2">
        <xsl:choose>
            <xsl:when test="matches(normalize-space(.), '^tao:A[-\+][1-5]_(beg|end)$')"/> <!-- delete font size information -->
