...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2026-01-27'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0091_change_004'"/>
...
<!--Added @n, @type in div2-->
    <xsl:template match="tei:body//tei:div2[not(@type)]">
        <xsl:copy>
            ...
            <xsl:attribute name="xml:id" select="concat('disp',$id)"/>
...
<!--TOC Added @target in ref-->   
    ...
    <!-- div1 "Booklevel" -->
    <xsl:template match="//tei:div1[@type='contents'][2]//tei:head//tei:ref[starts-with(., 'Liber') or starts-with(., 'LIBER')]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="lib-id"  select="count(preceding::tei:ref[starts-with(., 'Liber') or starts-with(., 'LIBER')])+1"/>
            <xsl:attribute name="target" select="concat('#b',$lib-id)"/>  
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
      
    <!-- ref from content to div2 "Disputatio-level" -->
    <xsl:template match="tei:div1[@type='contents'][2]//tei:head//tei:ref[starts-with(., '¶ Diſ') or starts-with(., 'Diſ')]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="disp-id"  select="count(preceding::tei:ref[starts-with(., '¶ Diſ') or starts-with(., 'Diſ')])+1"/>
            <xsl:attribute name="target" select="concat('#disp',$disp-id)"/>  
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
