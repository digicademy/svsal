...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2020-09-28'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0006_Vol03_change-011'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Added @n, @type, @xml:id to div(s) and @target to ref(s) in toc.'"/>
    ...
    <!--Adding @type, @n and @xml:id to div2(s) book 1 0009 - 0258.-->
     <xsl:template match="tei:body//tei:div1[@xml:id eq 'b1']//tei:div2[not(@type)]">
         <xsl:copy>
         <xsl:copy-of select="@*"/>
             <xsl:variable name="nchap" select="count(preceding-sibling::*//self::tei:div2[not(@type) and ancestor::tei:div1[@xml:id eq 'b1']])+1"/>
             <xsl:variable name="nbook" select="count(preceding::*//self::tei:div1[@type eq 'book'])+1"/>
             <xsl:attribute name="n" select="$nchap"/>
             <xsl:attribute name="type" select="'chapter'"/>
             <xsl:attribute name="xml:id" select="concat('cap',$nchap,'b',$nbook)"/> 
             <xsl:apply-templates/>
         </xsl:copy>
     </xsl:template>
    
    <!--Adding @type, @n and @xml:id to div2(s) in part 0259 - 0315.-->
     <xsl:template match="tei:body//tei:div1[@xml:id eq 'b2']//tei:div2[not(@type)]">
         <xsl:copy>
         <xsl:copy-of select="@*"/>
             <xsl:variable name="nchap" select="count(preceding-sibling::*/self::tei:div2[not(@type) and ancestor::tei:div1[@xml:id eq 'b2']])+1"/>
             <xsl:attribute name="n" select="$nchap"/>
             <xsl:attribute name="type" select="'chapter'"/>
    ...
    <!--Adding @target to ref(s) in content list 0006 - 0007-->

    <xsl:template match="tei:list[@xml:id eq 'listb1']//tei:ref">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="ref" select="count(preceding::*//self::tei:ref[ancestor::tei:list[@xml:id eq 'listb1']])+1"/> 
            <xsl:attribute name="target" select="concat('#cap',$ref,'b1')"/>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
    
    <!--Adding @target to ref(s) in content list 0260-->
    <xsl:template match="tei:list[@xml:id eq 'listb2']//tei:ref">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="ref" select="count(preceding::*//self::tei:ref[ancestor::tei:list[@xml:id eq 'listb2']])+1"/> 
            <xsl:attribute name="target" select="concat('#cap',$ref,'b2')"/>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
