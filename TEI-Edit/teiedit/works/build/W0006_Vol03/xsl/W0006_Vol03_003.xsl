...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0006_Vol03_change-012'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Added ref in summaries.'"/>
    ...
    <!-- Added ref in the first digit of items in summaries.   -->
     <xsl:template match="tei:list[@type eq 'summaries' and not(@xml:id)]//tei:item/text()[matches(.,'^\d')]" mode="ref">
    ...
    <xsl:variable name="noSpace">
        <xsl:apply-templates select="$ref" mode="noSpace"/>
    </xsl:variable>
    ...
    <xsl:template match="tei:list[@type eq 'summaries' and not(@xml:id)]//tei:ref" mode="noSpace">
        <xsl:copy>
        <xsl:copy-of select="@*"/>
