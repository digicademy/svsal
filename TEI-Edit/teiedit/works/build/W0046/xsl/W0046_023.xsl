...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2020-11-13'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0046_change_027'"/>
    <xsl:param name="editingDesc" as="xs:string" select="'Generated @xml:id after corrections.'"/>
    <xsl:template match="tei:teiHeader/tei:revisionDesc/tei:listChange">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:text>&#xa;                </xsl:text>
            <!--Update '2024-03-28' CR-->
            <xsl:element name="change">
                <xsl:attribute name="who" select="'#CR #auto'"/>
                <xsl:attribute name="when" select="'2024-03-28'"/>
                <xsl:attribute name="status" select="ancestor::tei:revisionDesc[1]/@status"/>
                <xsl:attribute name="xml:id" select="'W0046_change_028'"/>
                <xsl:attribute name="xml:lang" select="'en'"/>
                <xsl:value-of select="'Added xml:id(s) for quote-elements.'"/>
            </xsl:element>
            <xsl:text>&#xa;                </xsl:text>
            <xsl:element name="change">
