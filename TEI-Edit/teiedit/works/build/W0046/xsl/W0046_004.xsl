...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0046_change_009'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Added cross references in summaries'"/>
    ...
    <xsl:template match="tei:list[@type='summaries']//tei:ref[not(@target)]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <!--<xsl:message>Summaries, adding @target to ref using @xml:id in milestone</xsl:message>-->
                ...
                <xsl:variable name="milestoneId" select="following::tei:milestone[ancestor::tei:div1 = current()/ancestor::tei:div1][@n eq $refNumber][1]/@xml:id"/>
                <xsl:choose>
                    <!--<xsl:when test="not($milestoneId)">
                        <xsl:attribute name="target" select="'noTarget'"/>
                    </xsl:when>-->
                    <xsl:when test="not($milestoneId)"/>
                    <xsl:otherwise>
