...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2020-05-15'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0046_change_006'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Added @type to div(s) and @target to toc.'"/>
    ...
    <xsl:template match="tei:div1[ancestor::tei:body]">
         <xsl:copy>
             <xsl:copy-of select="@*"/>
             <xsl:attribute name="type" select="'chapter'"/>
             <xsl:attribute name="xml:id" select="concat('cap',count(preceding-sibling::tei:div1[ancestor::tei:body])+1)"/>
             <xsl:apply-templates/>
         </xsl:copy>
     </xsl:template>
    <xsl:template match="tei:ref[ancestor::tei:div1[@n eq '2']]">
        <xsl:copy>
            <xsl:attribute name="target" select="concat('#cap',count(preceding::tei:ref[ancestor::tei:div1[@n eq '2']])+1)"/>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
