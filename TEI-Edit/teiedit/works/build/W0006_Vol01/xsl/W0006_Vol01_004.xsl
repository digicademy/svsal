...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!--Added @targets to ref in summaries from milestones' @xml:id.
        09.07.20-->
    ...
    <xsl:template match="tei:list[@type='summaries' and not(ancestor::tei:div1[@xml:id eq 'part7'])]//tei:ref[not(@target)]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
    ...
    <xsl:template match="tei:div1[@xml:id eq 'part7']//tei:list[@type='summaries']//tei:ref[not(@target)]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <!--<xsl:message>Adding @target to ref in summaries.</xsl:message>-->
                <xsl:variable name="refNumber" as="xs:string" select="."/>
