...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2020-07-08'"/>
    <xsl:param name="changeId" as="xs:string" select="'Wxxxx_change_xx'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Added @n and @type to div(s).'"/>
    ...
    <!--Adding @type, @n and @xml:id to div1(s).-->
     <xsl:template match="tei:body//tei:div1[not(@type)]">
         <xsl:copy>
         <xsl:copy-of select="@*"/>
             <xsl:variable name="npart" select="count(preceding-sibling::*//self::tei:div1[not(@type) and ancestor::tei:body])+1"/>
