...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="editingDate" as="xs:string" select="'2018-07-09'"/>
    ...
    <xsl:variable name="volumeNumber" as="xs:integer">
        <xsl:choose>
            <xsl:when test="tei:TEI/tei:text/@n">
                <xsl:value-of select="xs:integer(/tei:text/@n)"/>
            </xsl:when>
            <xsl:otherwise>
    ...
    <!-- is the following template necessary? -->
            ...
            <xsl:when test="not(@xml:id)">
                <xsl:copy>
                    ...
                    <xsl:attribute name="xml:id">
                        <xsl:choose>
                            <xsl:when test="$volumeNumber eq 0">
                                <xsl:value-of select="concat($workId, '-meta-pa-', sal:mkHexId(current()))"/>
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:value-of select="concat($workId, '-', $volN, '-meta-pa-', sal:mkHexId(current()))"/>
                            </xsl:otherwise>
                        </xsl:choose>
                    </xsl:attribute>
                    <xsl:apply-templates/>
    ...
    <xsl:template match="tei:teiHeader/tei:revisionDesc/tei:listChange">
        <xsl:copy>
            ...
            <xsl:element name="change">
                ...
                <xsl:value-of select="'Generate new @xml:id.'"/>
            </xsl:element>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
    ...
    <xsl:template match="tei:text//tei:div">
        <xsl:copy>
            <xsl:copy-of select="@* except @xml:id"/> 
            <xsl:variable name="divN" as="xs:string" select="xs:string(count(ancestor::tei:div) + 1)"/>
            <xsl:attribute name="xml:id" select="sal:mkContainerElemId(current(), concat('d', $divN))"/>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
            ...
            <xsl:choose>
                <xsl:when test="@sameAs">
                    ...
                    <xsl:if test="not($targetPb)">
                        <xsl:message terminate="yes" select="'Error: key element variable for lb has no or an unacceptable value'"/>
                    </xsl:if>
    ...
    <xsl:template match="tei:text//tei:ref[@target and not((substring(@target, 15, 4) eq '_mn_') or (substring(@target, 15, 4) eq '-nm-'))]">
        <xsl:copy>
            ...
            <xsl:choose>
                <xsl:when test="$milestone or $p or $div or $list or $item or $mnote">
                    <xsl:choose>
                        ...
                        <xsl:when test="$div">
                            <!-- TODO: evaluate if this really works: -->
                            <xsl:variable name="divN" as="xs:string" select="xs:string(count(ancestor::tei:div) + 1)"/>
                            <xsl:attribute name="target" select="concat('#', sal:mkContainerElemId($div, concat('d', $divN)))"/>
                        </xsl:when>
                        <xsl:when test="$list">
    ...
    <xsl:template match="tei:text//tei:choice">
        <xsl:copy>
            ...
            <xsl:attribute name="xml:id">
                ...
                <xsl:choose>
                    <xsl:when test="child::*[self::tei:sic or self::tei:corr]">
                        <!-- correction of error in print source -->
                        <xsl:value-of select="concat($workId, '-', $volN, '-', $pageN, '-cc-', $hexId)"/>
                    </xsl:when>
                    <xsl:when test="child::*[self::tei:abbr or self::tei:expan]">
                        <!-- abbreviation expansion -->
                        <xsl:value-of select="concat($workId, '-', $volN, '-', $pageN, '-ce-', $hexId)"/>
                    </xsl:when>
                    <xsl:when test="child::*[self::tei:orig or self::tei:reg]">
                        <!-- normalization -->
    ...
    <xsl:function name="sal:mkContainerElemId" as="xs:string">
        ...
        <xsl:if test="not(string-length(concat($workId, '-', $volN, '-', $pageN, '-', $elemCode, '-', $hexId)) eq 21)">
    ...
    <xsl:function name="sal:mkHexId" as="xs:string">
        ...
        <xsl:variable name="hexString">
            <xsl:choose>
                <!-- calculate a hexadecimal 4-digit number based on preceding number of same elements; 
                ...
                <xsl:otherwise>
                    <xsl:value-of select="sal:decToHex(count($targetNode/preceding::node()[./name() eq $targetNode/name()]) + 1000)"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
