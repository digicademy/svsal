...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- make sure that we have defined a transformation for each possible element local-name(.) -->
    ...
    <xsl:template match="tei:note[@type='margin'  and not(@rend = ('asterisk', 'dagger'))]">
        <xsl:variable name="xmlId" as="xs:string" select="generate-id(.)"/>
        <xsl:copy>
            <xsl:attribute name="anchored" select="'false'"/>
            <xsl:attribute name="place" select="'margin'"/>
