...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0001_Vol01_change_025'"/>
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:if test="@break[. ne 'yes'] or @rendition">
                        ...
                        <xsl:message terminate="no" select="concat('ERROR: encountered unexpected @break or @rendition in element ', @xml:id)"/>
                    </xsl:if>
                    <xsl:copy-of select="@*"/> 
                </xsl:otherwise>
            </xsl:choose>
