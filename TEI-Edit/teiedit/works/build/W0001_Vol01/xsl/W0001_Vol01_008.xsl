...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2019-04-23'"/>
    <xsl:param name="editingDesc" as="xs:string" select="'Tagged special characters.'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0001_Vol01_change_015'"/>
    <xsl:param name="specialCharFile" as="xs:string" select="'../../../resources/chars/specialchars_2019-04-10.xml'"/>
                ...
                <xsl:analyze-string select="." regex="{$specialCharsRegex}"> 
                    ...
                    <xsl:non-matching-substring>
                        ...
                        <xsl:if test="matches(., '[&#x0100;-&#x10ffff;]')">
                            ...
                            <!--$thisText, ' | after element with xml:id: ', $thisText/preceding::*[@xml:id]/@xml:id-->
                        </xsl:if>
                        <xsl:value-of select="."/>
                    </xsl:non-matching-substring>
                </xsl:analyze-string>
