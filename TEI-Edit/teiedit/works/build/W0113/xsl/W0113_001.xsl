...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2020-03-25'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0113_change_004'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Added @n, @type, @xml:id to div2..'"/>
...
<!-- Added @target to ref(s).  -->
    <xsl:template match="//tei:list[@n eq '1']//tei:ref">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="target" select="count(preceding::tei:ref)+1"/>
            <xsl:attribute name="resp" select="'#auto'"/>
            <xsl:attribute name="target" select="concat('#chap',$target)"/>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
...
<!-- Added @xml:id and @type to div(s).  -->
    <xsl:template match="tei:div2[not(@type)]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:attribute name="n" select="count(preceding::tei:div2[ancestor::tei:div1[@type eq 'part']])+1"/>
            <xsl:attribute name="type" select="'chapter'"/>
            <xsl:attribute name="xml:id" select="concat('chap',count(preceding::tei:div2[ancestor::tei:div1[@type eq 'part']])+1)"/>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
