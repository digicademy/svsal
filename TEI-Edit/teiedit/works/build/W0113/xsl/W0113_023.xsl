...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2021-06-22'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0113_change_026'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Transformed note(s) with just (Nota. [roman] digits) into milestone(s) and added @n to marginal notes.'"/>
    <xsl:template match="tei:teiHeader/tei:revisionDesc/tei:listChange" mode="milestones">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:text>&#xa;                </xsl:text>
            ...
            <xsl:apply-templates mode="milestones"/>
        </xsl:copy>
    </xsl:template>

<!-- 1. Transformed note(s) with just (Nota.) and roman numbers into milestone(s).-->

    
    
    <xsl:variable name="milestones">
        <xsl:apply-templates select="/" mode="milestones"/>
    </xsl:variable>
    
    <!-- identity transform -->
    
    <xsl:template match="@*|node()" mode="milestones">
        <xsl:copy>
            <xsl:apply-templates select="@*|node()" mode="milestones"/>
        </xsl:copy>
    </xsl:template>
    
    <xsl:template match="tei:text//tei:note[tei:p[text()[matches(.,'^(.*?)(Nota[ :\.]+)([ijvx\.]+)')] or text()[matches(.,'^(Nota[ :\.]+)([ijvx\.]+)')]]]" mode="milestones">
            <xsl:element name="milestone">
                <xsl:variable name="numbers" select="tei:p/text()[matches(.,'([ijvx])')]"/>
                <xsl:attribute name="n">
                    <xsl:value-of select="translate($numbers,'Nota.: ','')"/>
                </xsl:attribute>
...
<!--2. Added @n to marginal notes.--> 
    
    <xsl:variable name="nNotes">
        <xsl:apply-templates select="$milestones" mode="nNotes"/>
    </xsl:variable>
    
    <!-- identity transform -->
    
    <xsl:template match="@*|node()" mode="nNotes">
        <xsl:copy>
            <xsl:apply-templates select="@*|node()" mode="nNotes"/>
        </xsl:copy>
    </xsl:template>
    
    <xsl:template match="tei:text//tei:note" mode="nNotes">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:attribute name="n" select="count(preceding::tei:note[ancestor::tei:text])+1"/>
            <xsl:apply-templates mode="nNotes"/>
        </xsl:copy>
    </xsl:template>
