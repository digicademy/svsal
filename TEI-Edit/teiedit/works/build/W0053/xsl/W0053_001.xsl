...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2025-06-11'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0053_change_005'"/>
    <xsl:param name="editingDesc" as="xs:string" select="'Added @type, @xml, @n to div(s).'"/>
    ...
    <!--Added type, n, xml:id(s) to div2-->
    <xsl:template match="tei:div2[not(@type)]">
    <xsl:copy>
        <xsl:copy-of select="@*"/>
        <xsl:variable name="n" select="count(preceding-sibling::tei:div2[not(@type)])+1"/>
        ...
        <xsl:attribute name="xml:id" select="concat('chap-',$n,'-p',ancestor::tei:div1/@n)"/>
        <xsl:apply-templates/>
    </xsl:copy>
    </xsl:template>

    <!--Added @type to list toc-->
    <xsl:template match="tei:div2[@type eq 'contents']//tei:list">
    <xsl:copy>
        <xsl:copy-of select="@*"/>
    ...
    <!--Add @targets for table of contents linked to div2(s)-->
    <xsl:template match="tei:div2[@type eq 'contents']//tei:list//tei:ref">
    <xsl:copy>
        <xsl:variable name="n" select="count(preceding::tei:ref[not(@target)][ancestor::tei:div2= current()/ancestor::tei:div2])+1"/>
        <xsl:attribute name="target" select="concat('#chap-',$n,'-p',ancestor::tei:div1/@n)"/>
        <xsl:apply-templates/>
    </xsl:copy>
    </xsl:template>

    <!--Added type, n, xml:id(s) to div3-->
    <xsl:template match="tei:div3[not(@type)]">
    <xsl:copy>
        <xsl:copy-of select="@*"/>
        <xsl:variable name="n" select="count(preceding-sibling::tei:div3[not(@type)])+1"/>
        <xsl:attribute name="n">
            <xsl:choose>
                <xsl:when test="@n">
                <xsl:value-of select="@n"/>
