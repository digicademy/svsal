    ...
    <xsl:function name="local:isDelimitedLeft" as="xs:boolean">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$startAtLineBeginning">
                <xsl:choose>
                    <xsl:when test="$node/preceding-sibling::node()[1]/self::*[local-name() = $delimiterElems] and $node/preceding-sibling::node()[1]/self::*[not(@break)]">
                        <xsl:value-of select="true()"/>
                    </xsl:when>
        ...
        <xsl:element name="choice">
            <xsl:attribute name="xml:id">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:value-of select="concat('ch',generate-id())"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:attribute>
