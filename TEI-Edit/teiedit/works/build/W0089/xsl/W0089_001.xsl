...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2020-12-30'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0089_change_004'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Added @type, @target, @unit and @xml:id attributes.'"/>
    ...
    <!--Added target for toc in list-->
    <xsl:template match="tei:list[@type eq 'contents']//tei:ref">
        <xsl:copy>
            <xsl:variable name="ChapNum" select="count(preceding::tei:ref[ancestor::tei:list[@type eq 'contents']])+1"/>
            <xsl:attribute name="target" select="concat('#chap',$ChapNum)"/>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
    
    <!--Added xml:id, n, type to div2(s)-->
    <xsl:template match="tei:div1[@type eq 'book']//tei:div2[not(@type)]">
        <xsl:copy>
            <xsl:variable name="ChapNum" select="count(preceding::tei:div2[ancestor::tei:div1[@type eq 'book']])+1"/>
            <xsl:attribute name="xml:id" select="concat('chap',$ChapNum)"/>
    ...
    <!--Added unit and xml:id(s) to milestone(s)-->
