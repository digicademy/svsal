...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="abbrExpanFile" as="xs:string" select="concat('../../../resources/config/abbr-', $lang, '.xml')"/>
    <!-- List of special characters -->
    <xsl:param name="specialCharFile" as="xs:string" select="'../../../resources/chars/specialchars_2025-08-21.xml'"/>
    
    <!-- The usual stuff: -->
    <xsl:param name="editors" as="xs:string" select="'#MAH  #DG #auto'"/>
    <xsl:param name="editingDate" as="xs:string" select="'2025-02-06'"/>
    ...
    <!--For xml:id(s) in Salamanca format.-->
    ...
    <xsl:function name="local:isDelimitedLeft" as="xs:boolean">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$startAtLineBeginning">
                <xsl:choose> 
                    <!--updated 04.05.23 #CR. Added  - and $node/preceding-sibling::node()[1]/self::*[not(@break)] - at the end of the line below.
                        This uses the @breaks to avoid tagging false positives at the beginning of lines-->
                    <xsl:when test="$node/preceding-sibling::node()[1]/self::*[not(@break)][local-name() = $delimiterElems] and $node/preceding-sibling::node()[1]/self::*[not(@break)]">
                        <xsl:value-of select="true()"/>
                    </xsl:when>
    ...
    <!-- 4.) Adding xml:id(s) to new choice elements. -->

    <xsl:variable name="IDS">
        <xsl:apply-templates select="$expandedSemicolonAbbr" mode="IDS"/>
        <xsl:message select="'INFO: adding xml:id(s) to new choice elements.'"/>
    </xsl:variable>
    ...
    <xsl:template match="tei:text//tei:choice" mode="IDS">
        <xsl:copy>
            <xsl:copy-of select="@* except @xml:id"/> 
            <xsl:attribute name="xml:id">
                <xsl:variable name="facs" as="xs:string" select="preceding::tei:pb[not(@sameAs)][1]/@facs"/>
                <xsl:variable name="pageN" select="substring($facs, string-length($facs) - 3, 4)"/>
                <xsl:variable name="hexId" select="local:mkHexId(current())"/>  
    ...
    <xsl:function name="local:mkHexId" as="xs:string">
        ...
        <xsl:variable name="hexString">
            <xsl:choose>
                ...
                <xsl:when test="$targetNode[ancestor::tei:note or ancestor::tei:label[@place eq 'margin']]">
                    <xsl:value-of select="local:decToHex(count($targetNode/preceding::node()[./name() eq $targetNode/name()]) + 2000)"/>
                </xsl:when>
                <xsl:otherwise>
                    ...
                    <xsl:value-of select="local:decToHex($preceding + ($ancestral * 300) + 1000)"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
    ...
    <xsl:function name="local:decToHex" as="xs:string">
        <xsl:param name="in" as="xs:integer"/>
        <xsl:sequence
        select="if ($in eq 0) then '0'
        else concat(if ($in ge 16) then local:decToHex($in idiv 16) else '',
    ...
    <!-- 5.) quality check, logging, and output -->
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="textLength3" as="xs:integer" select="string-length(replace(string-join($IDS//tei:text//text()[not(ancestor::tei:expan)], ''), '\s', ''))"/>
        ...
        <xsl:variable name="choiceDiff2" as="xs:integer" select="count($IDS//tei:choice[tei:expan]) - count($expandedAbbr//tei:choice[tei:expan])"/>
