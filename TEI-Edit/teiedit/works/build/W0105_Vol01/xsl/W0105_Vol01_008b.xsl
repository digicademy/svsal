...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="lang" as="xs:string" select="'la'"/> <!--select 'la' or 'es'--> 
    
    <xsl:param name="editors" as="xs:string" select="'#MAH #DG #auto'"/>
    <xsl:param name="editingDate" as="xs:string" select="'2025-02-06'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0105_vol01_change_11'"/>
    <xsl:param name="editingDesc" as="xs:string" select="concat('Tag unmarked breaks (', $lang, ') in marginals.')"/>
