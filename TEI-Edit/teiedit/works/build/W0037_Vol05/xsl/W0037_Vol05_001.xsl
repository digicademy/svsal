...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2020-12-17'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0037_Vol05_change_002'"/>
    <xsl:param name="editingDesc" as="xs:string" select="'Tagged @type, @n, @unit, @xml:id, @target in div(s), list(s), milestone(s).'"/>
...
<!--First round-->
    ...
    <!-- Tagged div2 @type, @n, @xml:id.   -->
     <xsl:template match="tei:div1[@type eq 'book']//tei:div2[not(@type)]" mode="round-1">
         <xsl:copy>
             <xsl:copy-of select="@*"/>
             <xsl:variable name="chapNu" select="count(preceding-sibling::*/self::tei:div2)+62"/>
    ...
    <xsl:template match="tei:div1[@type eq 'contents']//tei:ref" mode="round-1">
        <xsl:copy>
           <xsl:copy-of select="@*"/>
            <xsl:variable name="chapNu" select="count(preceding::tei:ref[ancestor::tei:div1[@type eq 'contents']])"/>
           <xsl:attribute name="target" select="concat('#chap',$chapNu+62)"/> 
           <xsl:apply-templates mode="round-1"/> 
        </xsl:copy>
    </xsl:template>
 
    <!--Tagged @type in lists-->
    <xsl:template match="tei:div1[@type eq 'contents']//tei:list[not(@type)]" mode="round-1">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
    ...
    <xsl:template match="tei:div1[@type eq 'book']//tei:list[not(@type)]" mode="round-1">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
    ...
    <xsl:template match="tei:div1[@type eq 'index']//tei:list[not(@type)]" mode="round-1">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
...
<!--Second round - Summaries, adding @target to ref using @xml:id in milestones.-->
    ...
    <xsl:template match="tei:list[@type='summaries']//tei:ref[not(@target)]" mode="round-2">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
