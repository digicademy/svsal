...
<!--Added missing pagination (pb/@n).   -->
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0078_change_008'"/>
    ...
    <xsl:template match="tei:body//tei:pb[not(@sameAs)]">
        <xsl:variable name="n" as="xs:integer" select="count(./preceding::tei:pb[not(@sameAs)][ancestor::tei:body])+3"/>
        <xsl:copy>
    ...
    <xsl:template match="tei:back//tei:pb[not(@sameAs)]">
        <xsl:variable name="n" as="xs:integer" select="count(./preceding::tei:pb[not(@sameAs)][ancestor::tei:back])+211"/>
        <xsl:copy>
            <xsl:copy-of select="@* except @n"/>
            <xsl:choose>
                <xsl:when test="contains(@n,'[')">
