...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0012_change_009'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Added @type(s) to div(s) and @target to ref.'"/>
...
<!--Added @type to div2-->
    <xsl:template match="tei:body//tei:div2[not(@type)]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="n" as="xs:integer" select="count(./preceding::tei:div2[ancestor::tei:div1[@type eq 'dict']])+1"/>
            <xsl:attribute name="resp" select="'#auto'"/>
            <xsl:attribute name="type" select="'entry'"/>
            <xsl:attribute name="xml:id" select="concat('ent',$n)"/>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>

<!--Added @target in list-->
    <xsl:template match="tei:back//tei:list//tei:head//tei:ref">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="n" as="xs:integer" select="count(./preceding::tei:head[ancestor::tei:list])+1"/>
            <xsl:attribute name="resp" select="'#auto'"/>
            <xsl:attribute name="target" select="concat('#ent',$n)"/>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>

<!--Added div3 and @type-->
    <xsl:template match="tei:p[ancestor::tei:div1[@type eq 'dict'] and not(parent::tei:div3)]">
