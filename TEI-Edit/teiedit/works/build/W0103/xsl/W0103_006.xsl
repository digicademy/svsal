...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDesc" as="xs:string" select="'Fixed hyphenated pb,cb,lb.'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0103_change_010'"/>
    ...
    <xsl:template match="tei:cb[not(@sameAs)]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:choose>
    ...
    <xsl:template match="tei:lb[@rendition]">
        <xsl:copy>
