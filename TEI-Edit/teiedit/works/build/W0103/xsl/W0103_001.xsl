...
<!--Added missing pagination (pb/@n).   -->
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2019-12-16'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0103_change_xxx'"/>
    ...
    <xsl:template match="tei:back//tei:pb[not(@sameAs)]">
        <xsl:variable name="n" as="xs:integer" select="count(./preceding::tei:pb[not(@sameAs)][ancestor::tei:back])+189"/>
        <xsl:copy>
