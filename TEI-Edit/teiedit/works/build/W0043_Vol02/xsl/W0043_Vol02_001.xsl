...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2021-11-16'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0043_Vol02_change_007'"/>
...
<!-- ref(s) in toc, books(s)-->
    <xsl:template match="tei:head//tei:ref[starts-with(.,'L')]" mode="first-part">
     <xsl:copy>
     <xsl:copy-of select="@*"/>
        <xsl:variable name="nRef" select="count(preceding::tei:head//tei:ref[starts-with(.,'Liber')])+11"/>
...
<!--Added @n, @type, @xml:id in div3 @type.-->

    <xsl:template match="tei:div3[not(@n) and ancestor::tei:div2[@type eq 'segment']]" mode="second-part">
     <xsl:copy>
     <xsl:copy-of select="@*"/>
...
<!--Normalized @n in chapters.-->

    <xsl:template match="tei:div3[@n and ancestor::tei:div2[@type eq 'segment']]" mode="second-part">
     <xsl:copy>
     <xsl:copy-of select="@n"/>
        <xsl:variable name="n" select="count(preceding::tei:div3[not(@type)])+1"/>
        <xsl:attribute name="xml:id" select="concat('cap',$n)"/>
        <xsl:attribute name="type" select="'chapter'"/>
        <xsl:apply-templates mode="second-part"/>  
     </xsl:copy>
     </xsl:template>

<!--3rd Part added @type and normalized @n to erroneus numbering of div4(s), 
    from 'PROBL(s) 38, SECTIO II, LIBER XI' b11.
    b12, incorrect from PROBL 24, LIBER XVI b16 incorrect from PROBL. 12
    As they were more than half of 'PROBL', all of them were set/normalized in [].
-->
    ...
    <xsl:template match="tei:div4[not(@type) and ancestor::tei:div2[@xml:id = ('seg2b11','seg2b12','seg2b16','seg2b18')]]" mode="third-part">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:attribute name="type" select="'section'"/>
            <xsl:variable name="problNum" select="count(preceding::*/self::tei:div4[not(@type)][ancestor::tei:div2[@xml:id = ('seg2b11','seg2b12','seg2b16','seg2b18')] = current()/ancestor::tei:div2[@xml:id = ('seg2b11','seg2b12','seg2b16','seg2b18')]])+1"/>
...
<!--4th Part added @type and @n to remaining div4(s).-->
    ...
    <xsl:template match="tei:div4[not(@type)]" mode="fourth-part">
        <xsl:copy>
            <xsl:attribute name="type" select="'section'"/>
