...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="tei:note[@type='marginalia' and @rend='noRef']">
        <xsl:variable name="xmlId" as="xs:string" select="generate-id(.)"/>
        <xsl:copy>
            <xsl:attribute name="anchored" select="'false'"/>
            <xsl:attribute name="place" select="'margin'"/>
    ...
    <xsl:template match="tei:note[@type='marginalia' and @rend = ('asterisk', 'dagger')]">
        <xsl:variable name="xmlId" as="xs:string" select="generate-id(.)"/>
        <xsl:if test="@rend = ('dagger', 'asterisk') and not(preceding-sibling::*[1]/self::tei:ref)">
            <xsl:element name="ref">
                <xsl:attribute name="type" select="'note-anchor'"/>
    ...
    <xsl:template match="*[not(self::tei:hi or self::tei:p or self::tei:seg or self::tei:milestone or self::tei:abbr or self::tei:note)][@rend or @rendition]">
