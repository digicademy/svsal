...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2020-06-19'"/>
    <xsl:param name="changeId" as="xs:string" select="'Wxxxx_change_xx'"/>
    ...
    <!--Adding pagination in front matter.-->
    <xsl:template match="tei:front//tei:pb[not(@sameAs)]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:attribute name="n">
                <xsl:variable name="currentPage" as="xs:string">
                    <xsl:number value="count(preceding::tei:pb[not(ancestor::tei:note and @sameAs)]) + 1" format="i"/>
                </xsl:variable>
        ...
        <xsl:copy>
            ...
            <xsl:attribute name="n">
                <xsl:variable name="n" as="xs:string">
                    <xsl:number value="count(preceding::tei:pb[ancestor::tei:back and not(ancestor::tei:note and @sameAs)]) + 633"/>
                </xsl:variable>
