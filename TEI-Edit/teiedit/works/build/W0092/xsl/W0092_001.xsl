...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2020-06-15'"/>
    <xsl:param name="changeId" as="xs:string" select="'Wxxxx_change_xx'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Added @n, @type, @xml:id to div(s) and @target to ref.'"/>
    ...
    <!--Adding @type, @n and @xml:id to div1(s).-->
     <xsl:template match="tei:body//tei:div1[not(@type)]">
         <xsl:copy>
         <xsl:copy-of select="@*"/>
             <xsl:variable name="ndisputation" select="count(preceding-sibling::*//self::tei:div1[not(@type) and ancestor::tei:body])+1"/>
             <xsl:attribute name="n" select="$ndisputation"/>
             <xsl:attribute name="type" select="'disputation'"/> 
             <xsl:attribute name="xml:id" select="concat('disputation',$ndisputation)"/>
             <xsl:apply-templates/>
         </xsl:copy>
     </xsl:template>
    ...
    <xsl:template match="tei:body//tei:div2[not(@type)]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="ndisputation" select="count(preceding::*//self::tei:div1[not(@type) and ancestor::tei:body])+1"/>
            <xsl:variable name="nSeg" select="count(preceding-sibling::*//self::tei:div2[not(@type) and ancestor::tei:div1[not(@type)]])+1"/>
            <xsl:attribute name="n" select="$nSeg"/>
            <xsl:attribute name="type" select="'segment'"/> 
            <xsl:attribute name="xml:id" select="concat('seg',$nSeg,'disp',$ndisputation)"/>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
    <!--Adding @target to ref for the toc..-->
    <xsl:template match="tei:div1[@type eq 'contents']//tei:ref">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="ref" select="count(preceding::*//self::tei:ref)+1"/> 
            <xsl:attribute name="target" select="concat('#disputation',$ref)"/>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
