...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2020-11-06'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0037_Vol03_change_002'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Normalized pagination.'"/>
    ...
    <!-- The pagination was completely normalized due to the many printing errors in the original.  -->
    <xsl:template match="tei:pb[not(@sameAs or ancestor::tei:front)]">
         <xsl:copy>
             <xsl:copy-of select="@facs|@xml:id"/>
             <xsl:variable name="NewN" select="count(preceding::tei:pb[not(@sameAs or ancestor::tei:front)])+1"/>
             <xsl:attribute name="n" select="concat('[',$NewN,']')"/>
             <xsl:apply-templates/>
         </xsl:copy>
     </xsl:template>
