...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0037_Vol03_change_009'"/>
    <xsl:param name="specialCharFile" as="xs:string" select="'../../../resources/chars/specialchars_2020-10-12.xml'"/>
                ...
                <xsl:analyze-string select="." regex="{$specialCharsRegex}"> 
                    ...
                    <xsl:non-matching-substring>
                        ...
                        <xsl:if test="matches(., '[&#x0100;-&#x10ffff;]')">
                            <xsl:message terminate="yes" select="concat('Error: found undeclared special character in text node: ', $thisText,'-', $thisText/preceding::tei:milestone[1]/@xml:id)"/><!--, ' | after element with xml:id: ', $thisText/preceding::*[@xml:id]/@xml:id)-->
                        </xsl:if>
                        <xsl:value-of select="."/>
                    </xsl:non-matching-substring>
                </xsl:analyze-string>
