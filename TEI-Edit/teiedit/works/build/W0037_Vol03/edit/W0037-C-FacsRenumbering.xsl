...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- 05.11.20 CR 
        Facsimiles and xml:idswere renumbered due to a Image sequence error.
         W0037_Vol03_v1-0.xml has the numbering from Textloop
         W0037_Vol03_001.xml has the corrected numbering according to the rearranged facsimiles. 
    -->
     ...
     <xsl:template match="tei:pb[not(@sameAS)]">
         <xsl:copy>
             ...
             <xsl:attribute name="facs">
                 <xsl:variable name="NewFacs">
                     <xsl:number value="count(preceding::tei:pb[not(@sameAs)]) + 1" format="0000"/></xsl:variable>
                 <xsl:value-of select="concat('W0037-C-',$NewFacs)"/>
             </xsl:attribute>
             <xsl:attribute name="xml:id">
                 <xsl:variable name="NewId">
                     <xsl:number value="count(preceding::tei:pb[not(@sameAs)]) + 1" format="0000"/></xsl:variable>
                 <xsl:value-of select="concat('W0037-C-',$NewId)"/>
             </xsl:attribute>
             <xsl:apply-templates/>
         </xsl:copy>
     </xsl:template>
    <xsl:template match="tei:cb">
        <xsl:copy>
            <xsl:variable name="pb" select="preceding::tei:pb[1]/@facs"/>
            <xsl:variable name="cb" select="count(preceding::tei:cb)+1"/>
            <xsl:attribute name="xml:id" select="concat($pb,'-cb-',$cb)"/>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
