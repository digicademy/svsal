...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2021-10-01'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0019_change_006'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Summaries, adding @target to ref using @xml:id in milestones.'"/>
    ...
    <xsl:template match="tei:div2[not(ancestor::tei:div1[@n=18]) and not(ancestor::tei:div1[@n=16]) and not(ancestor::tei:div1[@n=17])]//tei:list[@type='summaries']//tei:ref[not(@target)]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <!--<xsl:message>Summaries, adding @target to ref using @xml:id in milestone</xsl:message>-->
                <xsl:variable name="refNumber" as="xs:string" select="."/>
                <xsl:variable name="milestoneId" select="following::tei:milestone[ancestor::tei:div2[not(ancestor::tei:div1[@n=18]) and not(ancestor::tei:div1[@n=16]) and not(ancestor::tei:div1[@n=17])] = current()/ancestor::tei:div2[not(ancestor::tei:div1[@n=18]) and not(ancestor::tei:div1[@n=16]) and not(ancestor::tei:div1[@n=17])]][replace(@n, '[\[\]]', '') eq $refNumber][1]/@xml:id"/>
                <xsl:choose>
    ...
    <xsl:template match="tei:div1[@type = 'preface' ]//tei:list[@type='summaries']//tei:ref[not(@target)]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <!--<xsl:message>Summaries, adding @target to ref using @xml:id in milestone</xsl:message>-->
                <xsl:variable name="refNumber" as="xs:string" select="."/>
                <xsl:variable name="milestoneId" select="following::tei:milestone[ancestor::tei:div1[@type = 'preface'] = current()/ancestor::tei:div1[@type = 'preface']][replace(@n, '[\[\]]', '') eq $refNumber][1]/@xml:id"/>
                <xsl:choose>
                    <xsl:when test="not($milestoneId)">
                        <xsl:attribute name="target" select="'unknown'"/>
                    </xsl:when>
...
<xsl:template match="tei:div1[@n=18 or @n=16 or @n=17]//tei:list[@type='summaries']//tei:ref[not(@target)]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <!--<xsl:message>Summaries, adding @target to ref using @xml:id in milestone</xsl:message>-->
                <xsl:variable name="refNumber" as="xs:string" select="."/>
                <xsl:variable name="milestoneId" select="following::tei:milestone[ancestor::tei:div1[@n=18 or @n=16 or @n=17]][replace(@n, '[\[\]]', '') eq $refNumber][1]/@xml:id"/>
                <xsl:choose>
                    <xsl:when test="not($milestoneId)">
                        <xsl:attribute name="target" select="'unknown'"/>
                    </xsl:when>
