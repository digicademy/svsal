...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2026-06-03'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0027_Vol01_change_xx'"/>
    ...
    <xsl:template match="tei:div2[@type eq 'chapter']" mode="first-round">
    <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="n" select="count(preceding::tei:div2[ancestor::tei:div1 = current()/ancestor::tei:div1])+1"/>
            <xsl:variable name="id" select="count(preceding::tei:div2[ancestor::tei:body])+1"/>
            <xsl:attribute name="xml:id" select="concat('chap-',$id)"/>
    ...
    <!--Adde @targets to TOC-->
    <xsl:template match="//tei:list[@xml:id='list-b1']//tei:item//tei:ref[matches(.,'Cap|^\d+')]" mode="first-round">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="n" select="count(preceding::tei:ref[matches(.,'Cap|^\d+')])+1"/>
            <xsl:attribute name="target" select="concat('#chap-',$n)"/>
            <xsl:apply-templates mode="first-round"/>   
        </xsl:copy>
    </xsl:template>
    <xsl:template match="tei:list[ancestor::tei:body]" mode="first-round">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:attribute name="type">
                <xsl:choose>
                    <xsl:when test="child::tei:head[1][matches(.,'SVMMARIVM')]">
                    <xsl:value-of select="'summaries'"/>
                    </xsl:when>
                    <xsl:otherwise>
    ...
    <xsl:template match="tei:body//tei:div3[not(@type)]" mode="first-round">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:attribute name="type" select="'section'"/>
            <xsl:attribute name="n">
            <xsl:variable name="n" select="count(preceding::tei:div3[not(@type)][ancestor::tei:div2 = current()/ancestor::tei:div2])+1"/>
