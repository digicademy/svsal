...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="tei:pb">
        <xsl:copy>
            ...
            <xsl:attribute name="xml:id" select="generate-id(.)"/>
            <xsl:if test="following-sibling::*[1]/self::tei:cb and following-sibling::*[2]/self::tei:lb[@type eq 'nb']">
    ...
    <xsl:template match="tei:lb">
        <xsl:copy>
            <xsl:copy-of select="@resp|@cert|@change|@sameAs|@type|@xml:id"/> 
        </xsl:copy>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:copy>
                    ...
                    <xsl:if test="following-sibling::*[1]/self::tei:lb[@type eq 'nb'] and not(preceding-sibling::*[1]/self::tei:pb)">
                        <xsl:attribute name="rendition" select="'#noHyphen'"/>
                        <xsl:attribute name="break" select="'no'"/>
                    </xsl:if>
                    <xsl:if test="following-sibling::*[1]/self::tei:lb[@type eq 'nb'] and preceding-sibling::*[1]/self::tei:pb">
                        <xsl:attribute name="break" select="'no'"/>
                    </xsl:if>
                </xsl:copy>
            </xsl:otherwise>
    ...
    <xsl:template match="*[not(self::tei:hi or self::tei:p or self::tei:seg or self::tei:milestone or self::tei:abbr)][@rend or @rendition]">
    ...
    <xsl:template match="tei:unclear">
        <xsl:if test="@reason eq 'damage'">
            <xsl:element name="damage">
                <xsl:attribute name="agent" select="'ink'"/>
                <xsl:copy>
                    <xsl:attribute name="cert" select="'high'"/>
                    <xsl:attribute name="reason" select="'damage'"/>
                    <xsl:attribute name="resp" select="'#CR'"/>
                </xsl:copy>
            </xsl:element>
        </xsl:if>
    </xsl:template>
