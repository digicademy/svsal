...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2021-08-04'"/>
    <xsl:param name="editingDesc" as="xs:string" select="'Tagged special characters.'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0005_Vol02_change_017'"/>
                ...
                <xsl:analyze-string select="." regex="{$specialCharsRegex}"> 
                    ...
                    <xsl:non-matching-substring>
                        ...
                        <xsl:if test="matches(., '[&#x0100;-&#x10ffff;]')">
                            <xsl:message terminate="yes" select="concat('Error: found undeclared special character in text node: ', $thisText)"/><!--, ' previus line /preceding-sibling::node()[1] | after element with xml:id: ', $thisText/preceding::*[@xml:id]/@xml:id)-->
                        </xsl:if>
                        <xsl:value-of select="."/>
                    </xsl:non-matching-substring>
                </xsl:analyze-string>
