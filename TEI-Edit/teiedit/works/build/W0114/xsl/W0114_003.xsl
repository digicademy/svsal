...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2019-10-23'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0114_change_010'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Added @type, @xml:id to div(s) and @target to ref(s).'"/>
...
<!-- Added @type and @xml:id to div3(s).   -->    
     <xsl:template match="tei:div3">
         <xsl:copy>
             ...
             <xsl:attribute name="type" select="'entry'"/>
             <xsl:attribute name="resp" select="'#auto'"/>
             <xsl:attribute name="xml:id" select="concat('entry',count(preceding::tei:div3)+1)"/>
             <xsl:apply-templates/>
         </xsl:copy>
     </xsl:template>
    
<!-- Added @target to ref(s) not from excomunicatio.  -->
    <xsl:template match="tei:ref[not(ancestor::tei:list[@xml:id eq 'casus'])]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="target" select="count(preceding::tei:ref[not(ancestor::tei:list[@xml:id eq 'casus'])])+1"/>
            <xsl:attribute name="resp" select="'#auto'"/>
            <xsl:attribute name="target" select="concat('#entry',$target)"/>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>

<!-- Added @type and @xml:id to div5(s).   -->

    <xsl:template match="tei:div5[not(ancestor::tei:div4[@xml:id = ('Quanuis', 'simonia1')])]">
         <xsl:copy>
             <xsl:copy-of select="@*"/>
             <xsl:choose>
                 ...
                 <xsl:otherwise>
                    <xsl:attribute name="n" select="count(preceding::tei:div5)+1"/>
                 </xsl:otherwise>
             </xsl:choose>
             <xsl:attribute name="type" select="'section'"/>
             <xsl:attribute name="resp" select="'#auto'"/>
             <xsl:attribute name="xml:id" select="concat('section',count(preceding::tei:div5)+1)"/>
             <xsl:apply-templates/>
         </xsl:copy>
     </xsl:template>
 
<!-- Added @target to ref in excomunicatio-->

    <xsl:template match="tei:ref[not(parent::tei:head) and ancestor::tei:list[@xml:id eq 'casus']]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="target" select="count(preceding::tei:ref[not(parent::tei:head) and ancestor::tei:list[@xml:id eq 'casus']])+1"/>
            <xsl:attribute name="resp" select="'#auto'"/>
            <xsl:attribute name="target" select="concat('#section',$target)"/>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
