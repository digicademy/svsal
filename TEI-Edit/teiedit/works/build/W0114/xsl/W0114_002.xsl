...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2019-10-21'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0114_change_009'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Transformed hi(s) @rend'"/>
    ...
    <!-- identity transform 
    Upper-case the first letter in hi @rend eq huifu.
    -->
...
<!-- Deleted hi @rend eq huifu.   -->   
    
    <xsl:variable name="round1">
        <xsl:apply-templates select="/" mode="round1"/>
    </xsl:variable>
    
    <xsl:template match="tei:hi[@rend eq 'tao:huifu:']" mode="round1">
            <xsl:variable name="text" select="text()"/>
            <xsl:value-of select="upper-case($text)"/>
    </xsl:template>
    
<!-- Deleted hi @rend eq W+\d, negEZ.   -->
    ...
    <xsl:template match="tei:hi[@rend eq 'tao:negEZ']" mode="round2">
        <xsl:apply-templates mode="round2"/>
    </xsl:template>
    <xsl:template match="tei:hi[@rend eq 'tao:W']" mode="round2">
        <xsl:apply-templates mode="round2"/>
    </xsl:template>
    
<!-- Replaced hi @rend eq 'right' by seg  @rend eq 'gap' -->
    <xsl:template match="tei:div1[@type eq 'contents']//tei:hi[@rend eq 'right']" mode="round2">
        <xsl:element name="seg">
            <xsl:attribute name="rend" select="'gap'"/>
    ...
    <xsl:template match="/">
        ...
        <xsl:if test="$inWhitespace ne $outWhitespace">
            <xsl:message select="'Warning: amount of whitespace characters differs in input and output doc: '"/>
