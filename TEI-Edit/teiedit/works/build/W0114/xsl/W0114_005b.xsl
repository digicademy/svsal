...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="tei:q">
        <xsl:element name="quote"> 
            <xsl:copy-of select="@type|@resp|@cert|@change|@xml:lang|@xml:id|@next|@prev"/>
            <xsl:choose>
    ...
    <xsl:template match="tei:abbr[@rend eq 'choice']">
        <xsl:element name="choice">
            ...
            <xsl:attribute name="xml:id" select="concat('ch-',generate-id(.))"/>
            <xsl:apply-templates/>
        </xsl:element>
    </xsl:template>
    ...
    <xsl:template match="tei:abbr[@rend eq 'sic']">
        <xsl:element name="sic">
            <xsl:copy-of select="@* except @rend"/>
            <xsl:attribute name="xml:id" select="concat('sic-',generate-id(.))"/>
            <xsl:apply-templates/>
        </xsl:element>
    </xsl:template>
    ...
    <xsl:template match="*[not(self::tei:hi or self::tei:p or self::tei:seg or self::tei:abbr)][@rend or @rendition]">
    ...
    <xsl:template match="tei:g">
        <xsl:choose>
            ...
            <xsl:when test="@n eq '#char1e9c'">
                <xsl:copy>
                    <xsl:attribute name="ref" select="@n"/>
                    <xsl:value-of select="'ẜ'"/>
                </xsl:copy>
            </xsl:when>
            <xsl:otherwise>
