...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0114_change_008'"></xsl:param>
    ...
    <xsl:template match="@*|node()">
     ...
     <xsl:template match="tei:body//tei:pb[not(@sameAs)]">
         <xsl:copy>
             ...
             <xsl:when test="@n eq ''">
                 <xsl:variable name="rectoN" as="xs:string" select="preceding::tei:pb[1][not(@sameAs)]/@n"/>   
                 <xsl:attribute name="n" select="concat('[', $rectoN, ']v')"/>
             </xsl:when>
             <!-- recto pages without trailing "r" -->
