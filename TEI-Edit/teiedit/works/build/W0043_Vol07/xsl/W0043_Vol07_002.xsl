...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2024-11-07'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0043_Vol07_change_0004'"/>
...
<!-- 18: noster--> 
  ...
  <xsl:variable name="noster">
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="Abbr" as="node()*" select="$noster//tei:abbr[@rend eq 'abbr' and following-sibling::node()/self::tei:abbr[@rend eq 'expan' and matches(.,'[̃ ãāēẽõōũūꝓđ́]+')]]"/>
        <xsl:variable name="WrongExpansions" as="node()*" select="$noster//tei:abbr[@rend eq 'expan' and matches(.,'[̃ ãāēẽõōũūꝓđ́]+')]"/>
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:variable name="Expansions" as="xs:integer" select="count($noster//tei:abbr[@rend eq 'choice']//tei:abbr[@rend eq 'expan'])"/>
