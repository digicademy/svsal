...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2023-07-06'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0043_Vol07_change_007'"/>
...
<!--Added @n, @type, @xml:id in div1-->
    <xsl:template match="tei:div1[not(@type)]" mode="first-round">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="nDiv" select="count(preceding::tei:div1[not(@type)])+1"/>
            <xsl:attribute name="n" select="current()/(translate(@xml:id,'b',''))"/>
...
<!--Added @n, @type, @xml:id in div2-->
    <xsl:template match="tei:div2[not(@type)]" mode="first-round">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="nDiv" select="count(preceding::tei:div2[not(@type)][ancestor::tei:div1[starts-with(@xml:id,'b')] = current()/ancestor::tei:div1[starts-with(@xml:id,'b')]][not(@type)])+1"/>
...
<!--Added @n, @type, @xml:id in div3-->
    <xsl:template match="tei:div3[not(ancestor::tei:div1[@xml:id=('b45')])]" mode="first-round">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="nDiv" select="count(preceding::tei:div3[ancestor::tei:div1[starts-with(@xml:id,'b')] = current()/ancestor::tei:div1[starts-with(@xml:id,'b')]][not(@type)])+1"/>
            <xsl:variable name="book-id"  select="ancestor::tei:div1/@xml:id"/>
    ...
    <!--div3 b45 @n normalized.-->
    <xsl:template match="tei:div3[ancestor::tei:div1[@xml:id=('b45')]]" mode="first-round">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="nDiv" select="count(preceding::tei:div3[ancestor::tei:div1[starts-with(@xml:id,'b')] = current()/ancestor::tei:div1[starts-with(@xml:id,'b')]][not(@type)])+1"/>
            <xsl:variable name="book-id"  select="ancestor::tei:div1/@xml:id"/>
...
<!--Added @n, @type, @xml:id in div4 Liber -->
    <xsl:template match="tei:div4[not(ancestor::tei:div1[@xml:id=('b45')])][not(@type)]" mode="first-round">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="nDiv" select="count(preceding::tei:div4[not(@type)][ancestor::tei:div1 = current()/ancestor::tei:div1])+1"/>
            <xsl:choose>
            <xsl:when test="not(@n)">
                <xsl:attribute name="n" select="$nDiv"/>
            </xsl:when>
...
<!--Added @n, @type, @xml:id in div4 Liber 45 with most normalized @n(s)-->
    <xsl:template match="tei:div4[ancestor::tei:div1[@xml:id=('b45')]][not(@type)]" mode="first-round">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:variable name="nDiv" select="count(preceding::tei:div4[not(@type)][ancestor::tei:div1 = current()/ancestor::tei:div1])+1"/>
            <xsl:choose>
