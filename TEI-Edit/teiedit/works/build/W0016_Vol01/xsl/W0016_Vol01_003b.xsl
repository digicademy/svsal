...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="tei:div1|tei:div2|tei:div3|tei:div4|tei:div5|tei:div6|tei:div7">
        <xsl:element name="div">
            <xsl:copy-of select="@resp|@cert|@change|@n|@xml:lang|@type|@xml:id"/>
...
<xsl:otherwise>
    ...
    <xsl:template match="tei:ref">
        <xsl:copy>
            <xsl:copy-of select="@resp|@cert|@change|@n|@xml:lang|@target"/>
<xsl:message terminate="no" select="concat('Info: Here is the target-ref: ', @target)"/>
            <xsl:if test="@target">
