...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0016_Vol01_change_02'"></xsl:param>
...
<xsl:template match="tei:div1//tei:div2[@type='chapter']//tei:p[@rend eq 'h2']" mode="round-2">
   ...
   <xsl:template match="tei:back//tei:list[@xml:id]//tei:item" mode="round-2">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
  <xsl:variable name="listId" select="ancestor-or-self::tei:list/@xml:id"/>
 <xsl:variable name="nItem" select="count(preceding-sibling::tei:item[ancestor-or-self::tei:list[@xml:id]]) + 1"/>
