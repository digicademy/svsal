...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2021-01-21'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0005_Vol01_change_010'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Added crossed references between list (summaries) and milestone(s).'"/>
    ...
    <xsl:template match="tei:div1[@type eq 'preface']//tei:list[@type='summaries']//tei:ref[not(@type)]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
    ...
    <xsl:template match="tei:div1[@type eq 'book']//tei:list[@type='summaries']//tei:ref[not(@type)]">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <!--<xsl:message>Adding @target to ref in summaries.</xsl:message>-->
            <xsl:variable name="refNumber" as="xs:string" select="."/>
