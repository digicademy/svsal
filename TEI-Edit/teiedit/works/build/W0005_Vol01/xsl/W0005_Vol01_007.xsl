...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2021-02-17'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0005_Vol01_change_013'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Added pb-cb/@sameAs in note(s) from comment() tao:pb_in_note.'"/>
...
<!--Added pb-cb/@sameAs in marginal notes interrupted by pb.-->
    <xsl:template match="comment()[matches(normalize-space(.),'tao:pb_in_note')]">
        <xsl:variable name="id" select="ancestor::*/self::tei:note/following::tei:pb[1]/@xml:id"/>
        <xsl:variable name="cbid" select="ancestor::*/self::tei:note/following::tei:cb[1]/@xml:id"/>
        <xsl:element name="pb" >
             <xsl:attribute name="sameAs" select="concat('#',$id)"/>
            <xsl:attribute name="resp" select="'#auto'"/>
         </xsl:element>
        <xsl:element name="cb" >
            <xsl:attribute name="sameAs" select="concat('#',$cbid)"/>
            <xsl:attribute name="resp" select="'#auto'"/>
        </xsl:element>
        <xsl:apply-templates/>
    </xsl:template>
    ...
    <xsl:template match="/">
        ...
        <xsl:if test="$inPb ne $outPb or $inCb ne $outCb or $inLb ne $outLb">
            <xsl:message select="'Warn: different amount of input and output pb/cb/lb: '"/>
