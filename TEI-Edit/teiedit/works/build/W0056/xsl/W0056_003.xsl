...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0056_change_007'"></xsl:param>
    <xsl:param name="editingDesc" as="xs:string" select="'Generate @xml:id for marginal notes.'"/>
...
<xsl:template match="tei:note">
         <xsl:copy>
<xsl:copy-of select="@*"/>
             <xsl:variable name="note" select="count(preceding::tei:note)+1"/>
         <xsl:attribute name="xml:id" select="concat('note',$note)"/>
             <xsl:apply-templates/>
         </xsl:copy>
    </xsl:template>
