...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0051_Vol01_change_09'"/>
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    ...
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <xsl:message terminate="yes" select="concat('Error: encountered unforeseen column structure on page ', $thisLb/preceding-sibling::tei:pb/@facs, '. nb of cbStart: ',count($precedingCbStart/preceding-sibling::tei:lb), ' and cbEnd: ',  count($precedingCbEnd/preceding-sibling::tei:lb))"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:otherwise>
            </xsl:choose>
   ...
   <xsl:message select="concat('This is $marginalPages: ', $marginalPages)"/>
            <xsl:for-each select="$marginalPages//tei:lb">
                <xsl:copy>
                    <xsl:copy-of select="@*"/>
                    <xsl:variable name="prefix" as="xs:string">
                        <xsl:variable name="facs" as="xs:string" select="./parent::page/@facs"/>
                                    <xsl:variable name="precedingPage" as="xs:string" select="./parent::page/preceding-sibling::page[1]/@facs"/>
                        <xsl:if test="not(matches($facs, '^facs:W\d{4}(-[A-z])?-\d{4}$'))">
                            <xsl:message terminate="yes" select="concat('ERROR: @facs in $marginalPages//tei:lb has invalid value: ', $facs, ' the preceding facs is ', $precedingPage)"/>
                        </xsl:if>
                        <xsl:value-of select="replace($facs, '^facs:(W\d{4}(-[A-z])?)-(\d{4})$', '$1_$3')"/>
                    </xsl:variable>
