...
<xsl:attribute name="xml:id">
                    <xsl:variable name="xmlNode" select="key('anyNode',@xml:id)"/>

                    <xsl:choose>
                        <xsl:when test="$xmlNode">
                            <xsl:value-of select="generate-id($xmlNode)"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:message terminate="yes" select="concat('Error: could not find a matching element for ref/@xml:id: ', @xml:id)"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:attribute>
</xsl:if>
...
<xsl:attribute name="corresp">
                    <xsl:variable name="correspNode" select="key('anyNode', substring-after(@corresp, '#'))"/>

                    <xsl:choose>
                        <xsl:when test="$correspNode">
                            <xsl:value-of select="concat('#', generate-id($correspNode))"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:message terminate="yes" select="concat('Error: could not find a matching element for p/@corresp: ', @corresp)"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:attribute>
</xsl:if>
    ...
    <xsl:template match="tei:pb">
        <xsl:copy>
            ...
            <xsl:choose>
                <xsl:when test="@facs  and not(ancestor-or-self::tei:note)">
