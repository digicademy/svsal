...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0051_Vol01_change_10'"/>
...
<xsl:key name="ref" match="tei:ref" use="@xml:id"/>
    ...
    <xsl:template match="tei:text//tei:ref[@target and not((substring(@target, 15, 4) eq '_mn_') or (substring(@target, 15, 4) eq '-nm-'))]">
        <xsl:copy>
            ...
            <xsl:variable name="pb" select="key('pb', substring-after(@target, '#'))"/>
                <xsl:variable name="rf" select="key('ref', substring-after(@target, '#'))"/>
      <xsl:attribute name="xml:id">
                <xsl:value-of select="sal:mkAnchorElemId(., 'rf')"/>
            </xsl:attribute>

            <xsl:choose>
                <xsl:when test="$milestone or $p or $div or $list or $item or $mnote or $pb or $rf">
                    <xsl:choose>
                        ...
                        <xsl:when test="$pb">
                         ...
                         <xsl:when test="$rf">
                            <xsl:attribute name="target" select="concat('#', sal:mkContainerElemId($rf, 'rf'))"/>
                        </xsl:when>
                        <xsl:otherwise>
    ...
    <!--Added xml:id(s) for quote 28.03.24 CR-->
    <xsl:template match="tei:text//tei:quote">
        <xsl:copy>
            <xsl:copy-of select="@* except @xml:id"/> 
            <xsl:attribute name="xml:id" select="sal:mkContainerElemId(current(), 'qu')"/>
            <xsl:apply-templates/>
        </xsl:copy>
    </xsl:template>
    ...
    <xsl:template match="tei:text//tei:anchor">
        <xsl:copy>
            ...
            <xsl:if test="not($workId and $volN and $pageN and $hexId)">
                <xsl:message terminate="yes" select="concat('Error: required variables have null values: workID -> ', $workId, ' , voIN ->', $volN, ', $pageN ->', $pageN, ' and hexId ->', $hexId)"/>
            </xsl:if>
...
<xsl:message terminate="no" select="concat('tei:pb/@facs: ',$node/@xml:id)"/>
                </xsl:when>
                <xsl:otherwise>
