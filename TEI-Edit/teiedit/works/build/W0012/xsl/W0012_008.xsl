...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="changeId" as="xs:string" select="'W0012_change_024'"/>
    ...
    <xsl:template match="text()[matches(.,'[-=]\n?$') and following-sibling::node()[1][(self::tei:pb or self::tei:cb or self::tei:lb or self::tei:note or self::tei:milestone or self::tei:hi)]]" mode="breakLB">
        <xsl:choose>
    ...
    <xsl:template match="tei:pb" mode="breakPB_CB">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:choose>
                <xsl:when test="following-sibling::*[1]/self::tei:cb and following-sibling::*[2]/self::tei:lb[@break]">
                    <xsl:attribute name="break" select="'no'"/>
                    <xsl:attribute name="rendition" select="'#hyphen'"/>
                </xsl:when>
                ...
                <xsl:when test="following-sibling::*[1]/self::tei:lb[@break]">
                    <xsl:attribute name="break" select="'no'"/>
                    <xsl:attribute name="rendition" select="'#hyphen'"/>
                </xsl:when>
    ...
    <xsl:template match="/">
        ...
        <xsl:if test="$inChars ne $outChars">
            <xsl:message select="'Warning: amount of non-whitespace characters differs in input and output doc: '"/>
