...
<!--Added missing pagination (pb/@n).   -->
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2019-09-19'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0012_change_016'"/>
    <xsl:param name="editingDesc" as="xs:string" select="'Added @n to pb.'"/>
    ...
    <xsl:template match="tei:back//tei:pb[not(@sameAs)]">
        <xsl:variable name="n" as="xs:integer" select="count(./preceding::tei:pb[not(@sameAs)][ancestor::tei:back])+1267"/>
        <xsl:copy>
