...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="editingDate" as="xs:string" select="'2019-09-23'"/>
    <xsl:param name="changeId" as="xs:string" select="'W0012_change_018'"></xsl:param>
    ...
    <xsl:template match="tei:div3[not(@type)]">
            <xsl:if test="ancestor::tei:div2[@type eq 'book']">
                <xsl:copy>
                    <xsl:copy-of select="@*"/>
