...
(: INFO: in order for this query to work correctly and/or yielding correct results, the following preconditions must be satisfied:
...
(: 1.) Store lb and respective text nodes in a flat, but structured (XML) format: lb, followed by (all) text nodes for the respective line. :) 
let $mainNodes :=   <nodes>{
                    for $node in $text//node()[not(ancestor::*[@place eq 'margin'])] return 
                        if ($node/self::text() or $node/self::tei:lb) then
                            (: remove parts in languages other than Spanish :)
                            if ($node/ancestor::*[@xml:lang][1]/@xml:lang ne $lang) then ()
                            ...
                            (: tag note anchors specifically for identifying clear breaks later on (see corr:getBreak()); however, these nodes are not relevant for the corr. lists :)
                            else if ($node/self::text() and $node/ancestor::tei:ref[@type eq 'note-anchor']) then <text type="note-anchor">{$node}</text>
                            else
...
(: first round: create <line> strucutre and primarily identitify tokens :)
let $mainLines0 :=   <lines>{
                    for $n in $mainNodes/* return
                        if ($n/self::tei:lb) then
                            ...
                            (: getting the line's text, omitting note anchors :)
...
let $simpleTokens := distinct-values($mainLines//tok[@type eq 'simple'])
let $simpleEntries := 
    for $tok in $simpleTokens return
        ...
        let $tokenField :=   
            if ($wordformDict) then <td bgcolor="{$corr:color-ok}">{$token}</td>
        ...
        let $lemmaField :=  
            if ($lemma and $lemma ne 'N/A') then <td bgcolor="{$corr:color-ok}">{$lemma}</td>
            else <td bgcolor="{$corr:color-not-found}"> ? </td>
        let $freq := count($mainLines//tok[@type eq 'simple' and . eq $tok])
        let $freqField := <td bgcolor="{$corr:color-additional}">{string($freq)}</td>
        let $links := 
            for $tok in $mainLines//tok[@type eq 'simple' and . eq $tok] return
                let $id := $tok/parent::text/preceding-sibling::lb[1]/@xml:id
                let $pb := $text//tei:lb[@xml:id eq $id]/preceding::tei:pb[not(ancestor::*[@place eq 'margin']) and not(@sameAs)][1]
...
let $hyphenatedTokens := for $lb in $mainLines//lb[@break eq 'no'] return 
                                            <hyphToken>{
...
let $unpairedHyphenations := $mainLines//lb[@break eq 'no' and (parent::line/preceding-sibling::line[1]/text/tok[last()][not(@type eq 'hyphenated')]
                                                                 or following-sibling::text[1]/tok[1][not(@type eq 'hyphenated')])]
let $unmarkedHyphenations := $mainLines//lb[not(@break eq 'no') and (parent::line/preceding-sibling::line[1]/text/tok[last()][@type eq 'hyphenated']
...
let $hyphenEntries := 
    for $hyphSeq in $hyphenatedForms return
        ...
        let $links := for $lb in $allInstances return
                              ...
                              let $id := $text//tei:lb[@xml:id eq $thisId]/preceding::tei:lb[not(@sameAs) and not(ancestor::*[@place eq 'margin'])][1]/@xml:id
                              let $pb := $text//tei:lb[@xml:id eq $id]/preceding::tei:pb[not(ancestor::*[@place eq 'margin']) and not(@sameAs)][1]
                              let $imgUrl := corr:makeImageUrl($pb/@facs)
...
let $complexTokens := for $lb in $mainLines//lb[not(@break)][
...
(: #### START DEBUGGING SECTION :)
let $unpairedComplex := $mainLines//lb[not(@break) and (parent::line/preceding-sibling::line[1]/text/tok[last()][@type ne 'complex']
                                                        or following-sibling::text[1]/tok[1][@type ne 'complex']
                                                       )]
let $unmarkedComplex := $mainLines//lb[@break and (parent::line/preceding-sibling::line[1]/text/tok[last()][@type eq 'complex']
