...
let $entries := for $entry in /html/body/table/tr[@class = ('simple_not_found', 'hyphen_not_found')]
                    order by lower-case(replace($entry/*[1]/text(), ' [|-] ', ''))
                    return $entry

let $title := $workId || ': Unknown word forms, either occurring within lines or, already annotated as hyphenations, in the context of line breaks -> words should be edited manually in the text, if necessary (and added to the dictionary at some point)'
