...
let $entries := for $entry in /html/body/table/tr[@class eq 'comp_hyph']
                    order by lower-case(replace($entry/*[1]/text(), ' [|-] ', ''))
                    return $entry

let $title := $workId || ": Word forms separated by line breaks, which (according to the dictionary) are actually *one* word form -> breaks should be revised and possibly tagged as hyphenation (Umbruch) if the dictionary's guess is correct"
