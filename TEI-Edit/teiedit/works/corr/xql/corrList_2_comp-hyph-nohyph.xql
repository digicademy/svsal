...
let $entries := for $entry in /html/body/table/tr[@class eq 'comp_hyph_nohyph']
                    order by lower-case(replace($entry/*[1]/text(), ' [|-] ', ''))
                    return $entry

let $title := $workId || ': Word forms in the context of line breaks, with valid dictionary entries both for the complete (concatenated) word form as well as for its single parts -> breaks should be revised and possibly tagged as hyphenation (Umbruch), if required.'
